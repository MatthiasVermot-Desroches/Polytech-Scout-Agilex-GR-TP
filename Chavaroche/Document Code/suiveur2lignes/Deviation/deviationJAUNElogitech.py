...
class VisionLineNode(Node):
    ...
    def image_callback(self, msg):
        ...
        SEUIL_PRESENCE = 25  # Seuil de pixels blancs pour considérer la route présente dans une zone
        ...
        for nom, (y, (x_start, x_end)) in partitions.items():
            ...
            if masse > SEUIL_PRESENCE:
                ...
                if len(lignes) >= 2:
                    # Milieu entre les 2 lignes les plus proches du centre
                    lignes_triees = sorted(lignes, key=lambda x: abs(x - CENTRE_IMAGE))
                    l1, l2 = sorted(lignes_triees[:2])  # les 2 plus proches, triées gauche→droite
                    centres_x[nom] = (l1 + l2) // 2     # milieu de la route
                elif len(lignes) == 1:
                    # Une seule ligne — centre estimé avec la mémoire de largeur
                    unique_x     = lignes[0]
                    demi_largeur = self.largeur_route_memoire / 2
                    if unique_x > CENTRE_IMAGE:
                        centres_x[nom] = int(unique_x - demi_largeur)
                    else:
                        centres_x[nom] = int(unique_x + demi_largeur)
                else:
            ...
            # Point bleu = centre de la route (milieu entre les 2 lignes)
        ...
        if nb_lignes_L1 == 2:
            ...
            cv2.circle(debug_frame, (centre_L1, y), 5, (255, 0, 0), -1)
            cv2.circle(debug_frame, (centre_L2, y), 5, (255, 0, 0), -1)
