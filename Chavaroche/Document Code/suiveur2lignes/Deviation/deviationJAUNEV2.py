...
class VisionLineNode(Node):
    ...
    def image_callback(self, msg):
        ...
        height, width, _ = cv_image.shape  # width vaut normalement 848

        Y_L3 = 340 - 280  # Devient la ligne 60 dans la nouvelle image
        Y_L2 = 410 - 280  # Devient la ligne 130 dans la nouvelle image
        Y_L1 = 470 - 280  # Devient la ligne 190 dans la nouvelle image
        ...
        for x in range(1, width):
            h, s, v = segment_L2[x]
            # Utilisation des mêmes seuils élargis pour le scan par pixel
            if 10 <= h <= 30 and s >= 45 and v >= 50:  
                detections_L2.append(x)

        # Regrouper les pixels consécutifs
        if detections_L2:
            groupes = []
            groupe = [detections_L2[0]]
            for x in detections_L2[1:]:
                if x - groupe[-1] <= 5:
                    groupe.append(x)
                else:
            ...
            for g in groupes:
                cx = int(np.mean(g))
                # Dessin du point bleu sur l'image de débug cropée
                cv2.circle(debug_frame, (cx, Y_L2), 5, (255, 0, 0), -1)
        ...
        h_val = hsv[180, 700]
        self.get_logger().info(
            f"HSV ligne retour : H={h_val[0]} S={h_val[1]} V={h_val[2]}",
        ...
        for nom, (y, (x_start, x_end)) in partitions.items():
            # .copy() garantit qu'on isole la mémoire du segment sans polluer le reste
            segment = mask[y, x_start:x_end].copy() 
            
            # Compte strict des pixels blancs
            ...
            # --- RENDU VISUEL : Dessin des segments de scan ---
            # Couleur verte si de la route est détectée, rouge sinon
            couleur_zone = (0, 255, 0) if route_detectee[nom] == 1 else (0, 0, 255)
            cv2.line(debug_frame, (x_start, y), (x_end, y), couleur_zone, 2)
            
            # Dessin d'un petit cercle bleu sur le centre calculé de la zone
            # SEULEMENT si c'est une vraie détection (masse > SEUIL_PRESENCE)
        ...
        indices_blancs = np.where(segment_L1 == 255)[0] + X_C[0]
        ...
        if len(indices_blancs) > 0:
            ...
            for g in Groupes:
                if len(g) >= 5:
                    nb_lignes_trouvees += 1
                    lignes_x.append(int(np.mean(g)))
                    
                    # --- RENDU VISUEL : Entourer les lignes détectées sur L1 ---
                    # Dessine un point jaune flashy sur chaque ligne identifiée sur L1
                    cv2.circle(debug_frame, (int(np.mean(g)), Y_L1), 8, (0, 255, 255), -1)

        msg_single = Bool()
        msg_single.data = (nb_lignes_trouvees == 1)
        self.single_line_pub.publish(msg_single)

        # Calcul de l'erreur globale
        if nb_lignes_trouvees == 2:
            self.derniere_ligne_perdue = "AUCUNE"
            centre_route_reel = (lignes_x[0] + lignes_x[1]) // 2
            
            # ✅ CALIBRATION : Mémoriser la largeur réelle de la route observée
            largeur_observee = abs(lignes_x[1] - lignes_x[0])
            # Filtrer les mesures aberrantes (si largeur change trop brutalement)
            if 200 < largeur_observee < 600:
                # Moyenne exponentielle pour lisser les variations
                self.largeur_route_memoire = 0.7 * self.largeur_route_memoire + 0.3 * largeur_observee
                self.get_logger().info(
                    f"Largeur route calibrée: {self.largeur_route_memoire:.0f}px",
                    throttle_duration_sec=2.0)
            
            err_L1 = float(centre_route_reel - 424)
            err_L2 = float(centres_x['L2'] - 424)
            erreur_calculer = (POIDS_L1 * err_L1) + (POIDS_L2 * err_L2)
            
            # --- RENDU VISUEL : Ligne de cible (Nominal) ---
            # Une ligne blanche relie le centre calculé au centre théorique
            cv2.line(debug_frame, (centre_route_reel, Y_L1), (424, Y_L1), (255, 255, 255), 2)

        elif nb_lignes_trouvees == 1:
            unique_x = lignes_x[0]
            ...
            if unique_x > 424:
                ...
                erreur_calculer = float((unique_x - demi_largeur) - 424)
            else:
        ...
        # --- RENDU VISUEL : Ligne verticale centrale de l'écran (Référence 424) ---
        cv2.line(debug_frame, (424, 0), (424, height), (255, 0, 255), 1) # Ligne rose au centre
        
        # Affichage du mode sur l'image
        texte_mode = f"Lignes visibles: {nb_lignes_trouvees} | Last Lost: {self.derniere_ligne_perdue}"
