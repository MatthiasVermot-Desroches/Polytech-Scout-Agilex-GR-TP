...
typedef struct _NetworkControlEntry {
	...
	uint64_t waittime;
	int failedconn;
	MTAILQ_ENTRY(_NetworkControlEntry)
	link;
} NetworkControlEntry;
 ...
 */
void clientClosed(PhidgetNetConnHandle nc) {
	NetworkControlEntry *nce;
 ...
 */
void clientAuthFailed(PhidgetNetConnHandle nc) {
	NetworkControlEntry *nce;
 ...
 */
void clientProtocolFailure(PhidgetNetConnHandle nc) {
	NetworkControlEntry *nce;
...
#if ZEROCONF_SUPPORT
void releaseMDNSControlEntries(ZeroconfListenerHandle handle) {
