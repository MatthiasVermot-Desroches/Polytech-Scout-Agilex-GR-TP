...
RB_HEAD(clients, _ClientControl)
clients;
...
RB_HEAD(servers, _IPhidgetServer)
servers;
...
runAcceptServer(void *arg) {
	...
	for (;;) {
		...
		mos_netop_setrecvtimeout(MOS_IOP_IGNORE, &cs, 10000);
...
int validServerName(const char *srvname) {
	const char *c, *b;
