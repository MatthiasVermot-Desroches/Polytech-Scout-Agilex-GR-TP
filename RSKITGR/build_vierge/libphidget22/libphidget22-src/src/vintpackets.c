...
PhidgetReturnCode
VINTPacketStatusCodeDeviceCode_to_PhidgetReturnCode(VINTPacketStatusCode code, const PhidgetUniqueDeviceDef *udd) {
	...
	case PHIDUID_DCC1120:
	#if PHIDUID_DCC1110_SUPPORTED
	case PHIDUID_DCC1110:
	#endif // PHIDUID_DCC1110_SUPPORTED
	#if PHIDUID_DCC1130_SUPPORTED
	case PHIDUID_DCC1130:
	#endif // PHIDUID_DCC1130_SUPPORTED
		switch (code) {
		case VINTPacketStatusCode_DCC11X0_BADPOWER:
			return (EPHIDGET_BADPOWER);
		case VINTPacketStatusCode_DCC11X0_POWERCYCLE:
			return (EPHIDGET_POWERCYCLE);
		case VINTPacketStatusCode_DCC11X0_HALL_ERROR:
			return (EPHIDGET_HALLSENSOR);
		case VINTPacketStatusCode_DCC11X0_INIT:
			return (EPHIDGET_BADCURRENT);
		}
		break;
#endif // PHIDUID_DCC1120_SUPPORTED
#if PHIDUID_DCC1020_SUPPORTED
	case PHIDUID_DCC1020:
	#if PHIDUID_DCC1010_SUPPORTED
	case PHIDUID_DCC1010:
	#endif // PHIDUID_DCC1010_SUPPORTED
	#if PHIDUID_DCC1030_SUPPORTED
	case PHIDUID_DCC1030:
	#endif // PHIDUID_DCC1030_SUPPORTED
		switch (code) {
		case VINTPacketStatusCode_DCC10X0_BADPOWER:
			return (EPHIDGET_BADPOWER);
		case VINTPacketStatusCode_DCC10X0_POWERCYCLE:
			return (EPHIDGET_POWERCYCLE);
		case VINTPacketStatusCode_DCC10X0_INIT:
			return (EPHIDGET_BADCURRENT);
		}
		break;
...
recvGENERICVINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_VINT_GENERIC:
		return (bridgeSendToChannel(ch, BP_PACKET, 1, "%*R", (int)len, buf));
...
sendFirmware(mosiop_t iop, PhidgetChannelHandle ch, const uint8_t *data, size_t dataLen) {
	...
	// Send the data
	offset = 0;
	bridgeSendToChannel(ch, BP_PROGRESSCHANGE, 1, "%g", 0.0);
	lastProgress = 0;
	do {
		...
		if (progress - lastProgress >= 0.01) {
			bridgeSendToChannel(ch, BP_PROGRESSCHANGE, 1, "%g", progress);
	...
	if (lastProgress != 1)
		bridgeSendToChannel(ch, BP_PROGRESSCHANGE, 1, "%g", 1.0);
...
recvFIRMWARE_UPGRADE_STM8S(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	Phidget_DeviceHardwareID deviceID;
	int version;
	int pkt;
	...
	case PHIDCHUID_STM8S_FIRMWARE_UPGRADE_100:
		...
		case VINT_BOOTLOADER_PACKET_TYPE_DEVICEINFO:
			deviceID = (Phidget_DeviceHardwareID)(((unsigned short)buf[0] << 4) | ((buf[1] & 0xF0) >> 4));
			version = ((unsigned short)(buf[1] & 0x0F) * 100) + buf[2];
			return (bridgeSendToChannel(ch, BP_DEVICEINFO, 2, "%d%d", deviceID, version));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
static PhidgetReturnCode
sendFIRMWARE_UPGRADE_STM32G0_BLBug(PhidgetChannelHandle ch, BridgePacket *bp) {
	PhidgetReturnCode res;
	size_t base64Len;
	void *base64;
	int off;
	...
	case PHIDCHUID_STM32G0_FIRMWARE_UPGRADE_110:
		...
		case BP_SENDFIRMWARE:
			...
			if (base64Len > 26624) {
				logerr("Firmware length must be <=26kb for this bootloader version.");
				mos_free(base64, base64Len);
				return (MOS_ERROR(bp->iop, EPHIDGET_INVALIDARG, "Firmware length must be <=26kb for this bootloader version."));
			}
...
static PhidgetReturnCode
recvFIRMWARE_UPGRADE_STM32G0_BLBug(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	Phidget_DeviceHardwareID deviceID;
	int version;
	int pkt;
...
static PhidgetReturnCode
sendFIRMWARE_UPGRADE_STM32F3(PhidgetChannelHandle ch, BridgePacket *bp) {
	PhidgetReturnCode res;
	size_t base64Len;
	void *base64;
	int off;
...
static PhidgetReturnCode
recvFIRMWARE_UPGRADE_STM32F3(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	Phidget_DeviceHardwareID deviceID;
	int version;
	int pkt;
...
recv1014_3_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_1014_DIGITALOUTPUT_800_VINT:
		...
		case VINT_PACKET_TYPE_DIGITALOUTPUT_FAILSAFE:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_FAILSAFE, "Failsafe procedure initiated."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_1024_V2_VINT_OBSOLETE_SUPPORTED
static PhidgetReturnCode
send1024_V2_VINT_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recv1024_V2_VINT_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_1024_V2_VINT_OBSOLETE_SUPPORTED */
...
send1024_V2_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_1024_RFID_200_VINT:
		...
		case BP_WRITE:
			...
			PhidgetUnlock(ch);
			if (ret != EPHIDGET_OK) {
				if (ret == EPHIDGET_INVALID)
					return (MOS_ERROR(bp->iop, EPHIDGET_INVALID, "Antenna must be enabled to write."));
				else
					return ret;
			}

			expectedString = (const char *)&buffer[2];
			return RFIDSupport_waitForTag(bp->iop, ch, expectedString, 700, NULL);
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recv1024_V2_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_1024_RFID_200_VINT:
		...
		case VINT_PACKET_TYPE_RFID_TAG:
			...
			PhidgetUnlock(ch);
			return bridgeSendToChannel(ch, BP_TAG, 2, "%s%d", &buf[1], buf[0]);
		case VINT_PACKET_TYPE_RFID_TAG_LOST:
			return bridgeSendToChannel(ch, BP_TAGLOST, 2, "%s%d", &buf[1], buf[0]);
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvADP1001_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_ADP1001_DATAADAPTER_100_VINT:
		...
		case VINT_PACKET_TYPE_DATAADAPTER_VOLTAGE_ERROR:
			PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_VOLTAGEERROR, "Voltage Error Detected");
			return (EPHIDGET_OK);
		default:
	...
	case PHIDCHUID_ADP1001_DIGITALINPUT_RI_100_VINT:
		...
		case VINT_PACKET_TYPE_DIGITALINPUT_STATECHANGE:
			state = buf[0] ? 1 : 0;
			return (bridgeSendToChannel(ch, BP_STATECHANGE, 1, "%d", state));
		case VINT_PACKET_TYPE_DIGITALINPUT_INVALID:
			PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_INVALIDSTATE, "Invalid State Detected");
			return EPHIDGET_OK;
		}
	default:
		MOS_PANIC("Invalid Channel UID");
...
recvADP_SERIAL_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_ADP_SERIAL_DIGITALINPUT_100_VINT:
		...
		case VINT_PACKET_TYPE_DIGITALINPUT_STATECHANGE:
			...
			return (bridgeSendToChannel(ch, BP_STATECHANGE, 1, "%d", state));
		case VINT_PACKET_TYPE_DIGITALINPUT_INVALID:
			PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_INVALIDSTATE, "Channel Invalidated. This means some other aspect of the device is making use of the channel.");
			return (EPHIDGET_OK);
		default:
			MOS_PANIC("Invalid Channel UID");
		}
...
recvVCP1002(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_VCP1002_VOLTAGEINPUT_110:
		...
		case VINT_PACKET_TYPE_VOLTAGEINPUT_VOLTAGECHANGE:
			...
			return (bridgeSendToChannel(ch, BP_VOLTAGECHANGE, 1, "%g", voltage));
		case VINT_PACKET_TYPE_VOLTAGEINPUT_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Saturation Detected."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_TMP1300_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendTMP1300_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvTMP1300_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_TMP1300_OBSOLETE_SUPPORTED */
...
recvTMP1300(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TMP1300_TEMPERATURESENSOR_IR_100:
		...
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			...
			return (bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, 1, "%g", temperature));
...
sendTMP1200(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_100:
		...
		case BP_SETRTDWIRESETUP:
			buf[0] = getBridgePacketInt32(bp, 0);
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_RTD_WIRESETUP, buf, 1);
			// Need to update the other channels because RTD Wire Setup is board-wide
			if (ret == EPHIDGET_OK) {
				phid->RTDWireSetup = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_RTDWIRESETUPCHANGE, 1, "%d", phid->RTDWireSetup);
			}
			return (ret);
		default:
			MOS_PANIC("Unexpected packet type");
...
recvTMP1200(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_100:
		...
		case VINT_PACKET_TYPE_RESISTANCEINPUT_RESISTANCECHANGE:
			...
			return (bridgeSendToChannel(ch, BP_RESISTANCECHANGE, 1, "%g", resistance));
...
sendTMP1200_1(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_300:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
...
recvTMP1200_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_300:
		...
		case VINT_PACKET_TYPE_RESISTANCEINPUT_RESISTANCECHANGE:
			...
			if (len > 4) {
				if (resistance > 0)
					PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGEHIGH, "Resistance is too high to be accurately measured");
				else
					return PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: Check polarity of connection");
			}
			return (bridgeSendToChannel(ch, BP_RESISTANCECHANGE, 1, "%g", resistance));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_BADCONNECTION:
			result = (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection"));
			if (result != EPHIDGET_OK)
				return result;
			return (bridgeSendToChannel(ch, BP_RESISTANCECHANGE, 1, "%g", NAN));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_TMP1200_TEMPERATURESENSOR_RTD_300:
		...
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			...
			if (len > 4) {
				if (temperature > 0)
					PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGEHIGH, "Temperature is too high to be accurately measured");
				else
					PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGELOW, "Temperature is too low to be accurately measured");
			}
			temperature = round_double(temperature, 3);
			return (bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, 1, "%g", temperature));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_SATURATION:
			result = (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGE, "Temperature is unknown"));
			if (result != EPHIDGET_OK)
				return result;
			return (bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, 1, "%g", NAN));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_BADCONNECTION:
			if (buf[0])
				result = (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: Check polarity of connection"));
			else
				result = (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: RTD is likely disconnected"));
			if (result != EPHIDGET_OK)
				return result;
			return (bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, 1, "%g", NAN));
		default:
...
#if PHIDUID_TMP1200_2_SUPPORTED
static PhidgetReturnCode
sendTMP1200_2(PhidgetChannelHandle ch, BridgePacket *bp) {
	PhidgetVINTDeviceHandle phid = (PhidgetVINTDeviceHandle)ch->parent;
	PhidgetReturnCode ret;
	uint8_t buf[4];
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_400:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_TMP1200_TEMPERATURESENSOR_RTD_400:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
...
static PhidgetReturnCode
recvTMP1200_2(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	PhidgetReturnCode result;
	double temperature;
	double resistance;
	int pkt;
	...
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_400:
		switch (pkt) {
		case VINT_PACKET_TYPE_RESISTANCEINPUT_RESISTANCECHANGE:
			resistance = unpackfloat(buf);
			if (isnan(resistance) == 0)
				resistance = round_double(resistance, 5);
			if (len > 4) {
				if (isnan(resistance))
					PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection");
				else if (resistance > 0)
					PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGEHIGH, "Resistance is too high to be accurately measured");
				else {
					PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection: Check polarity of connection");
				}
			}
			return (bridgeSendToChannel(ch, BP_RESISTANCECHANGE, 1, "%g", resistance));
		default:
	...
	case PHIDCHUID_TMP1200_TEMPERATURESENSOR_RTD_400:
		...
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			...
			temperature = round_double(temperature, 5);
			return (bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, 1, "%g", temperature));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_SATURATION:
			if (len > 0) {
				result = (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGE, "Temperature is unknown"));
...
recvTMP1101_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TMP1101_VOLTAGEINPUT_200:
		...
		case VINT_PACKET_TYPE_VOLTAGEINPUT_BADCONNECTION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADCONNECTION, "Bad Connection"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_STC1005_0_SUPPORTED
	case PHIDCHUID_STC1005_STEPPER_100:
#endif
#if PHIDUID_STC1005_0_110_SUPPORTED
...
recvSTC1003_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_STC1003_STEPPER_200:
		...
		case VINT_PACKET_TYPE_STEPPER_STATUSUPDATE:
			...
			// Stopped
			stopped = buf[12] ? 1 : 0;
			returnCode = bridgeSendToChannel(ch, BP_POSITIONCHANGE, 1, "%l", motorPosition);
			if (returnCode != EPHIDGET_OK)
				return returnCode;
			returnCode = bridgeSendToChannel(ch, BP_VELOCITYCHANGE, 1, "%g", velocity);
			if (returnCode != EPHIDGET_OK)
				return returnCode;
			returnCode = PhidgetStepper_getIsMoving((PhidgetStepperHandle)ch, &moving);
			if (returnCode == EPHIDGET_OK && stopped && moving)
				returnCode = bridgeSendToChannel(ch, BP_STOPPED, 0, NULL);
			return returnCode;
		case VINT_PACKET_TYPE_MOTORCONTROLLER_FAILSAFE:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_FAILSAFE, "Failsafe procedure initiated."));
		case VINT_PACKET_TYPE_STEPPER_POWER_ERROR:
			if (buf[0])
				return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADPOWER, "Supply Voltage above expected range"));
			else
				return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADPOWER, "Supply Voltage below expected range. Possible overcurrent condition."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_STC1003_VOLTAGEINPUT_200:
...
#if PHIDUID_STC1005_0_SUPPORTED
static PhidgetReturnCode
sendSTC1005_0(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendSTC1003_1(ch, bp);
}

static PhidgetReturnCode
recvSTC1005_0(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int64_t motorPosition;
	double velocity;
	double voltage;
	int stopped;
	...
	case PHIDCHUID_STC1005_STEPPER_110:
		...
		case VINT_PACKET_TYPE_STEPPER_POWER_ERROR:
			if (buf[0])
				...
				return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADPOWER, "Supply Voltage below expected range. Possible overcurrent condition."));
		case VINT_PACKET_TYPE_STEPPER_FAULT_ERROR:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_FAULT, "Fault condition detected. Motor is disengaged and position is lost. Re-engage and re-initialize your system."));
		case VINT_PACKET_TYPE_STEPPER_OVERTEMPERATURE:
			PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_FAULT, "Fault condition detected. Motor is disengaged and position is lost. Re-engage and re-initialize your system.");
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OVERTEMP, "Overtemperature Condition Detected."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_STC1005_VOLTAGEINPUT_100:
...
#if PHIDUID_STC1005_0_110_SUPPORTED
static PhidgetReturnCode
sendSTC1005_0_110(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendSTC1005_0(ch, bp);
}

static PhidgetReturnCode
recvSTC1005_0_110(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recvSTC1005_0(ch, buf, len);
}
#endif
	...
	case PHIDCHUID_STC1001_STEPPER_100:
		...
		case VINT_PACKET_TYPE_STEPPER_STATUSUPDATE:
			...
			if (stopped && (stopped == moving)) {
				return (bridgeSendToChannel(ch, BP_STOPPED, 0, NULL));
			}
			return returnCode;
		case VINT_PACKET_TYPE_MOTORCONTROLLER_FAILSAFE:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_FAILSAFE, "Failsafe procedure initiated."));
		case VINT_PACKET_TYPE_STEPPER_POWER_ERROR:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADPOWER,
		...
		case VINT_PACKET_TYPE_SOUNDSENSOR_DBCHANGE:
			...
			if (saturatedFlag) {
				...
				if (fabs(lastdB - dB) > changeTrigger) {
					...
					return (bridgeSendToChannel(ch, BP_DBCHANGE, 4, "%g%g%g%10G", dB, dBA, dBC, finaloctaves));
				} else
					return EPHIDGET_OK;
			}
		default:
...
recvSAF1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_SAF1000_POWERGUARD_100:
		...
		case VINT_PACKET_TYPE_POWERGUARD_ENERGYDUMP:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_ENERGYDUMP, "Energy Dump in Progress."));
			break;
		...
		case VINT_PACKET_TYPE_POWERGUARD_OVERVOLTAGE:
			if (buf[0])
				return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OVERVOLTAGE, "Overvoltage Condition Detected."));
		case VINT_PACKET_TYPE_POWERGUARD_POWERON:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OK, "All error conditions have been resolved."));
			break;
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvTP_DIAGNOSER_0(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_TP_DIAGNOSER_RESISTANCEINPUT_100:
		...
		case VINT_PACKET_TYPE_RESISTANCEINPUT_RESISTANCECHANGE:
			...
			return (bridgeSendToChannel(ch, BP_RESISTANCECHANGE, 1, "%g", value));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_RCC1000_1_SUPPORTED
...
recvRCC1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_RCC1000_RCSERVO_100:
		...
		case VINT_PACKET_TYPE_RCSERVO_ARRIVED_AT_TARGET_POSITION:
			...
			return (bridgeSendToChannel(ch, BP_TARGETPOSITIONREACHED, 1, "%g", (motorPosition / 16.0)));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_RCC1000_RCSERVO_110:
...
#if PHIDUID_RCC1000_1_SUPPORTED
static PhidgetReturnCode
sendRCC1000_1(PhidgetChannelHandle ch, BridgePacket *bp) {

	return (sendRCC1000_Failsafe(ch, bp));
}

static PhidgetReturnCode
recvRCC1000_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	return (recvRCC1000_Failsafe(ch, buf, len));
}
#endif
...
recvPRE1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_PRE1000_PRESSURESENSOR_100:
		...
		case VINT_PACKET_TYPE_PRESSURESENSOR_PRESSURECHANGE:
			...
			return (bridgeSendToChannel(ch, BP_PRESSURECHANGE, 1, "%g", pressure));
...
recvHUM1100(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_HUM1100_VOLTAGERATIOINPUT_100:
		...
		case VINT_PACKET_TYPE_VOLTAGERATIOINPUT_VOLTAGERATIOCHANGE:
			...
			return (bridgeSendToChannel(ch, BP_VOLTAGERATIOCHANGE, 1, "%g", value));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	default:
...
recvMOT1101(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_MOT1101_SPATIAL_100:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			if (fireSaturation == 0) {
				...
				ret = bridgeSendToChannel(ch, BP_SPATIALDATA, 4, "%3G%3G%3G%g", accel, gyro, compass, timestamp);
			} else {
				if (fireSaturation & 0x01)
					ret = PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Accelerometer Saturation Detected.");
				if (fireSaturation & 0x02)
					ret = PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Gyroscope Saturation Detected.");
				if (fireSaturation & 0x04)
					ret = PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Magnetometer Saturation Detected.");
				else
	...
	case PHIDCHUID_MOT1101_MAGNETOMETER_100:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			return (bridgeSendToChannel(ch, BP_FIELDSTRENGTHCHANGE, 2, "%3G%g", compass, timestamp));
		case VINT_PACKET_TYPE_AXES_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Saturation Detected."));
		default:
			MOS_PANIC("Unexpected packet type");
	...
	case PHIDCHUID_MOT1101_GYROSCOPE_100:
		...
		case VINT_PACKET_TYPE_AXES_AXESUPDATE:
			...
			return (bridgeSendToChannel(ch, BP_ANGULARRATEUPDATE, 2, "%3G%g", gyro, timestamp));
		case VINT_PACKET_TYPE_AXES_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Saturation Detected."));
		default:
			MOS_PANIC("Unexpected packet type");
	...
	case PHIDCHUID_MOT1101_ACCELEROMETER_100:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			timestamp = (double)unpack32(buf + 6);
			return (bridgeSendToChannel(ch, BP_ACCELERATIONCHANGE, 2, "%3G%g", accel, timestamp));
			break;
		case VINT_PACKET_TYPE_AXES_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Acceleration Saturation"));
			break;
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_MOT1102_1_SUPPORTED
...
#if PHIDUID_MOT1102_1_SUPPORTED
	case PHIDCHUID_MOT1102_MAGNETOMETER_300:
#endif
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
...
#if PHIDUID_MOT1102_1_SUPPORTED
	case PHIDCHUID_MOT1102_GYROSCOPE_300:
#endif
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
...
#if PHIDUID_MOT1102_1_SUPPORTED
	case PHIDCHUID_MOT1102_ACCELEROMETER_300:
#endif
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
	...
	case PHIDCHUID_MOT1102_SPATIAL_300:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			if (fireSaturation == 0) {
				...
				ret = bridgeSendToChannel(ch, BP_SPATIALDATA, 4, "%3G%3G%3G%g", accel, gyro, compass, timestamp);
				if (ret == EPHIDGET_OK)
					ret = bridgeSendToChannel(ch, BP_SPATIALALGDATA, 2, "%4G%g", quaternion, timestamp);
			} else {
				if (fireSaturation & 0x01)
					ret = PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Accelerometer Saturation Detected.");
				if (fireSaturation & 0x02)
		...
		case VINT_PACKET_TYPE_AXES_INVALID:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_INVALIDSTATE, "The gyroscope has received invalid data and will recover shortly."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_MOT1102_1_SUPPORTED
static PhidgetReturnCode
sendMOT1102_1(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendMOT1102(ch, bp);
}
#endif

#if PHIDUID_MOT1102_1_SUPPORTED
static PhidgetReturnCode
recvMOT1102_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recvMOT1102(ch, buf, len);
}
#endif

static PhidgetReturnCode
sendMOT0110_SETCORRECTIONPARAMETERS(PhidgetChannelHandle ch, BridgePacket *bp) {
	float firmware_userMagnetometerTransform[6];
	float firmware_userMagnetometerOffset[3];
	float firmware_userMagnetometerGain[3];
	float firmware_userMagField;
...
#if PHIDUID_MOT0110_VINT_SUPPORTED
static PhidgetReturnCode
sendMOT0110_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	PhidgetVINTDeviceHandle phid = (PhidgetVINTDeviceHandle)ch->parent;
	PhidgetReturnCode ret;
	uint8_t buf[24];
	...
	case PHIDCHUID_MOT0110_SPATIAL_100_VINT:
		...
		case BP_SAVECORRECTIONPARAMETERS:
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_MAGNETOMETER_SAVECORRECTIONPARAMETERS, NULL, 0));
		case BP_SETCORRECTIONPARAMETERS:
			return (sendMOT0110_SETCORRECTIONPARAMETERS(ch, bp));
		case BP_ZEROSPATIALALGORITHM:
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SPATIAL_ZERO_AHRS, NULL, 0));
		case BP_SETSPATIALALGORITHM:
			buf[0] = getBridgePacketInt32(bp, 0);
		...
		case BP_SETHEATINGENABLED:
			buf[0] = getBridgePacketInt32(bp, 0);
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SPATIAL_SET_HEATING, buf, 1);
			// Need to update the other channels because heating enabled is board-wide
			if (ret == EPHIDGET_OK) {
				phid->heatingEnabled = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_HEATINGENABLEDCHANGE, 1, "%d", phid->heatingEnabled);
			}
			return (ret);
		default:
			MOS_PANIC("Unexpected packet type");
	...
	case PHIDCHUID_MOT0110_MAGNETOMETER_100_VINT:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_MOT0110_GYROSCOPE_100_VINT:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
	...
	case PHIDCHUID_MOT0110_ACCELEROMETER_100_VINT:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
...
static PhidgetReturnCode
recvMOT0110_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	PhidgetReturnCode ret;
	double temperature;
	double timestamp;
	double quaternion[4];
	...
	case PHIDCHUID_MOT0110_SPATIAL_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			if (len > 40) {
				if (buf[40] & 0x20)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more spatial readings is out of range");
			}

			for (i = 0; i < 3; i++) {
				tmpFlt = unpackfloat(buf + (i * 4));
				if (!isnan(tmpFlt))
					compass[i] = round_double(tmpFlt, 5);

				tmpFlt = unpackfloat(buf + 12 + (i * 4));
				if (!isnan(tmpFlt))
					accel[i] = round_double(tmpFlt, 5);

				tmpFlt = unpackfloat(buf + 24 + (i * 4));
				if (!isnan(tmpFlt))
					gyro[i] = round_double(tmpFlt, 5);
			}

			timestamp = (double)unpack32(buf + 36);

			ret = bridgeSendToChannel(ch, BP_SPATIALDATA, 4, "%3G%3G%3G%g", accel, gyro, compass, timestamp);
			return (ret);
		case VINT_PACKET_TYPE_AXES_AXESUPDATE:

			quaternion[0] = (double)unpackfloat(buf + 0);
			quaternion[1] = (double)unpackfloat(buf + 4);
			quaternion[2] = (double)unpackfloat(buf + 8);
			quaternion[3] = (double)unpackfloat(buf + 12);

			timestamp = (double)unpack32(buf + 16);

			ret = bridgeSendToChannel(ch, BP_SPATIALALGDATA, 2, "%4G%g", quaternion, timestamp);

			return (ret);
			break;
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
			ret = bridgeSendToChannel(ch, BP_DATAINTERVALCHANGE, 1, "%u", interval);
			return ret;
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_MOT0110_MAGNETOMETER_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			if (len > 16) {
				if (buf[16] & 0x04)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more magnetometer readings is out of range");
			}

			compass[0] = unpackfloat(buf);
			compass[1] = unpackfloat(buf + 4);
			compass[2] = unpackfloat(buf + 8);
			timestamp = (double)unpack32(buf + 12);
			for (i = 0; i < 3; i++) {
				if (!isnan(compass[i]))
					compass[i] = round_double(compass[i], 6);
			}
			return (bridgeSendToChannel(ch, BP_FIELDSTRENGTHCHANGE, 2, "%3G%g", compass, timestamp));
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
	...
	case PHIDCHUID_MOT0110_GYROSCOPE_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXESUPDATE:
			if (len > 16) {
				if (buf[16] & 0x02)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more gyroscope readings is out of range");
			}
			gyro[0] = unpackfloat(buf);
			gyro[1] = unpackfloat(buf + 4);
			gyro[2] = unpackfloat(buf + 8);
			for (i = 0; i < 3; i++) {
				if (!isnan(gyro[i]))
					gyro[i] = round_double(gyro[i], 6);
			}
			timestamp = (double)unpack32(buf + 12);
			return (bridgeSendToChannel(ch, BP_ANGULARRATEUPDATE, 2, "%3G%g", gyro, timestamp));
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
	...
	case PHIDCHUID_MOT0110_ACCELEROMETER_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			if (len > 16) {
				if (buf[16] & 0x01)
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more accelerometer readings is out of range");
			}
			accel[0] = unpackfloat(buf);
			accel[1] = unpackfloat(buf + 4);
			accel[2] = unpackfloat(buf + 8);
			timestamp = (double)unpack32(buf + 12);
			for (i = 0; i < 3; i++) {
				if (!isnan(accel[i]))
					accel[i] = round_double(accel[i], 6);
			}
			return (bridgeSendToChannel(ch, BP_ACCELERATIONCHANGE, 2, "%3G%g", accel, timestamp));
			break;
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
	...
	case PHIDCHUID_MOT0110_TEMPERATURESENSOR_100_VINT:
		...
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			if (len > 4) {
				switch (buf[4] & 0x18) {
				case 0x08:
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGELOW, "Temperature is too low to be accurately measured");
					break;
				case 0x10:
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Temperature is too high to be accurately measured");
					break;
				case 0x18:
					ret = PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "Temperature is unknown");
					break;
				}
			}
			temperature = unpackfloat(buf);
...
#if PHIDUID_MOT0100_VINT_SUPPORTED
static PhidgetReturnCode
sendMOT0100_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[24];
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_MOT0100_ACCELEROMETER_100_VINT:
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 2));
...
static PhidgetReturnCode
recvMOT0100_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	double timestamp;
	double accel[3];
	int pkt;
	int i;
	...
	case PHIDCHUID_MOT0100_ACCELEROMETER_100_VINT:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			if (len > 16) {
				if (buf[16] & 0x01)
					PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "One or more accelerometer readings is out of range");
			}
			accel[0] = unpackfloat(buf);
			accel[1] = unpackfloat(buf + 4);
			accel[2] = unpackfloat(buf + 8);
...
recvMOT1100(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_MOT1100_ACCELEROMETER_200:
		...
		case VINT_PACKET_TYPE_AXES_AXISVALUECHANGE:
			...
			timestamp = (double)unpack32(buf + 6);
			return (bridgeSendToChannel(ch, BP_ACCELERATIONCHANGE, 2, "%3G%g", acc, timestamp));
			break;
		case VINT_PACKET_TYPE_AXES_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Acceleration Saturation"));
			break;
...
recvLUX1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_LUX1000_LIGHTSENSOR_100:
		...
		case VINT_PACKET_TYPE_LIGHTSENSOR_ILLUMINANCECHANGE:
			...
			return (bridgeSendToChannel(ch, BP_ILLUMINANCECHANGE, 1, "%g", illuminance));
...
#if PHIDUID_LCD1100_1_SUPPORTED
	case PHIDCHUID_LCD1100_LCD_200:
#endif
		switch (bp->vpkt) {
...
#if PHIDUID_LCD1100_1_SUPPORTED
static PhidgetReturnCode
sendLCD1100_1(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendLCD1100(ch, bp);
}

static PhidgetReturnCode
recvLCD1100_1(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recvLCD1100(ch, buf, len);
}
#endif
...
recvHUM1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_HUM1000_HUMIDITYSENSOR_100:
		...
		case VINT_PACKET_TYPE_HUMIDITYSENSOR_HUMIDITYCHANGE:
			...
			return (bridgeSendToChannel(ch, BP_HUMIDITYCHANGE, 1, "%g", humidity));
...
recvHIN1100(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_HIN1100_VOLTAGERATIOINPUT_100:
		...
		case VINT_PACKET_TYPE_VOLTAGERATIOINPUT_VOLTAGERATIOCHANGE:
			...
			return (bridgeSendToChannel(ch, BP_VOLTAGERATIOCHANGE, 1, "%g", axis));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_HIN1100_DIGITALINPUT_100:
		...
		case VINT_PACKET_TYPE_DIGITALINPUT_STATECHANGE:
			value = buf[0] ? 1 : 0;
			return (bridgeSendToChannel(ch, BP_STATECHANGE, 1, "%d", value));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	default:
...
recvHIN1001(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_HIN1001_CAPACITIVETOUCH_WHEEL_100:
		...
		case VINT_PACKET_TYPE_CAPACITIVETOUCH_TOUCHVALUECHANGE:
			...
			if (touched)
				return (bridgeSendToChannel(ch, BP_TOUCHINPUTVALUECHANGE, 1, "%g", value));
			return (bridgeSendToChannel(ch, BP_TOUCHINPUTEND, 0, NULL));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvHIN1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_HIN1000_CAPACITIVETOUCH_100:
		...
		case VINT_PACKET_TYPE_CAPACITIVETOUCH_TOUCHVALUECHANGE:
			touched = buf[0] ? 1 : 0;
			if (touched)
				return (bridgeSendToChannel(ch, BP_TOUCHINPUTVALUECHANGE, 1, "%g", (double)touched));
			return (bridgeSendToChannel(ch, BP_TOUCHINPUTEND, 0, NULL));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_ENC1000_ENCODER_100:
		...
		case VINT_PACKET_TYPE_ENCODER_ENCODERCHANGE:
			...
			if (indexTriggered)
				...
				indexPosition = 0;
			return (bridgeSendToChannel(ch, BP_POSITIONCHANGE, 4, "%d%g%c%d", positionChange, timeChange / 1000000.0, indexTriggered, indexPosition));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_ENC1001_SUPPORTED
static PhidgetReturnCode
sendENC1001(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[4];
...
static PhidgetReturnCode
recvENC1001(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int indexPosition;
	uint8_t indexTriggered;
	int positionChange;
	uint64_t timeChange;
	...
	case PHIDCHUID_ENC1001_ENCODER_100:
		...
		case VINT_PACKET_TYPE_ENCODER_ENCODERCHANGE:
			positionChange = unpack32(buf);
			timeChange = ((uint64_t)(unpack32(buf + 4)) * 1000) / 64; // Time comes back as counts at 64MHz, convert to ns
			indexTriggered = buf[8];
			if (indexTriggered)
				indexPosition = unpack32(buf + 9);
			else
		...
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
			return bridgeSendToChannel(ch, BP_DATAINTERVALCHANGE, 1, "%u", interval);
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#endif // PHIDUID_ENC1001_SUPPORTED
...
recvHIN1101(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_HIN1101_ENCODER_100:
		...
		case VINT_PACKET_TYPE_ENCODER_ENCODERCHANGE:
			positionChange = unpack32(buf);
			timeChange = ((uint64_t)(unpack32(buf + 4)) * 1000) / 16; // Time comes back as counts at 16MHz, convert to ns
			return (bridgeSendToChannel(ch, BP_POSITIONCHANGE, 4, "%d%g%c%d", positionChange, timeChange / 1000000.0, 0, 0));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_DST1200_DISTANCESENSOR_100:
		...
		case VINT_PACKET_TYPE_DISTANCESENSOR_SONARREFLECTIONSUPDATE:
			...
			if (reflections_count == 0)
				return (bridgeSendToChannel(ch, BP_SONARUPDATE, 3, "%*U%*U%u", DISTANCE_MAX_REFLECTIONS, reflections_distance, DISTANCE_MAX_REFLECTIONS, reflections_strength, reflections_count));

			ret = (bridgeSendToChannel(ch, BP_SONARUPDATE, 3, "%*U%*U%u", DISTANCE_MAX_REFLECTIONS, reflections_distance, DISTANCE_MAX_REFLECTIONS, reflections_strength, reflections_count));
			if (ret == EPHIDGET_OK)
				return (bridgeSendToChannel(ch, BP_DISTANCECHANGE, 1, "%u", reflections_distance[0]));
			else
				return ret;
		case VINT_PACKET_TYPE_DISTANCESENSOR_DISTANCECHANGE:
			distance = unpack32(&buf[0]);
			return (bridgeSendToChannel(ch, BP_DISTANCECHANGE, 1, "%u", distance));
...
recvDST1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DST1000_DISTANCESENSOR_100:
		...
		case VINT_PACKET_TYPE_DISTANCESENSOR_SATURATION:
			...
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGE, "Sensor value is ouside the valid range for this sensor."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
#if PHIDUID_DCC1020_SUPPORTED
static PhidgetReturnCode
sendDCC1020(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[8] = {0};
	...
	switch (ch->UCD->uid) {
	#if PHIDUID_DCC1010_SUPPORTED
	case PHIDCHUID_DCC1010_CURRENTINPUT_100:
	#endif // PHIDUID_DCC1010_SUPPORTED
	case PHIDCHUID_DCC1020_CURRENTINPUT_100:
	#if PHIDUID_DCC1030_SUPPORTED
	case PHIDCHUID_DCC1030_CURRENTINPUT_100:
	#endif // PHIDUID_DCC1030_SUPPORTED
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
	...
	#if PHIDUID_DCC1010_SUPPORTED
	case PHIDCHUID_DCC1010_VOLTAGEINPUT_100:
	#endif // PHIDUID_DCC1010_SUPPORTED
	case PHIDCHUID_DCC1020_VOLTAGEINPUT_100:
	#if PHIDUID_DCC1030_SUPPORTED
	case PHIDCHUID_DCC1030_VOLTAGEINPUT_100:
	#endif // PHIDUID_DCC1030_SUPPORTED
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
	...
	#if PHIDUID_DCC1010_SUPPORTED
	case PHIDCHUID_DCC1010_TEMPERATURESENSOR_IC_100:
	#endif // PHIDUID_DCC1010_SUPPORTED
	case PHIDCHUID_DCC1020_TEMPERATURESENSOR_IC_100:
	#if PHIDUID_DCC1030_SUPPORTED
	case PHIDCHUID_DCC1030_TEMPERATURESENSOR_IC_100:
	#endif // PHIDUID_DCC1030_SUPPORTED
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
	...
	#if PHIDUID_DCC1010_SUPPORTED
	case PHIDCHUID_DCC1010_ENCODER_100:
	#endif // PHIDUID_DCC1010_SUPPORTED
	case PHIDCHUID_DCC1020_ENCODER_100:
	#if PHIDUID_DCC1030_SUPPORTED
	case PHIDCHUID_DCC1030_ENCODER_100:
	#endif // PHIDUID_DCC1030_SUPPORTED
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
	...
	case PHIDCHUID_DCC1030_DCMOTOR_100:
		...
		case BP_SETSURGECURRENTLIMIT:
			packfloat(buf, (getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DCMOTOR_SETSURGECURRENTLIMIT, buf, 4));
		case BP_SETFANMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_MOTORCONTROLLER_FAN, buf, 1));
		case BP_SETDATAINTERVAL:
		...
		case BP_SETINDUCTANCE:
			packfloat(buf, (getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DCMOTOR_SETINDUCTANCE, buf, 4));
		case BP_DECELERATIONTYPE:
			buf[0] = (uint8_t)getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DCMOTOR_SETDECELERATIONTYPE, buf, 1));
		case BP_SETFAILSAFEBRAKINGDUTYCYCLE:
			packfloat(buf, (getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DCMOTOR_SETFAILSAFEBRAKINGSTRENGTH, buf, 4));
		case BP_SETFAILSAFECURRENTLIMIT:
			packfloat(buf, (getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DCMOTOR_SETFAILSAFECURRENTLIMIT, buf, 4));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	#if PHIDUID_DCC1010_SUPPORTED
	...
	case PHIDCHUID_DCC1030_MOTORPOSITIONCONTROLLER_100:
		...
		case BP_SETENABLEEXPECTEDPOSITION:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_ENABLEEXPECTEDPOS, buf, 1));
		case BP_SETFAILSAFEBRAKINGDUTYCYCLE:
			packfloat(buf, (getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DCMOTOR_SETFAILSAFEBRAKINGSTRENGTH, buf, 4));
		case BP_SETFAILSAFECURRENTLIMIT:
...
#if PHIDUID_DCC1010_SUPPORTED
static PhidgetReturnCode
sendDCC1010(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendDCC1020(ch, bp);
}
#endif /* PHIDUID_DCC1010_SUPPORTED */

#if PHIDUID_DCC1030_SUPPORTED
static PhidgetReturnCode
sendDCC1030(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendDCC1020(ch, bp);
}
#endif /* PHIDUID_DCC1030_SUPPORTED */
...
recvDCC1100(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DCC1100_BLDCMOTOR_100:
		...
		case VINT_PACKET_TYPE_BLDCMOTOR_MOTORSTATUSUPDATE:
			...
			if (PhidgetBLDCMotor_getLastBrakingStrength((PhidgetBLDCMotorHandle)ch) != brakingDutyCycle) {
				result = bridgeSendToChannel(ch, BP_BRAKINGSTRENGTHCHANGE, 1, "%g", brakingDutyCycle);
				if (result != EPHIDGET_OK)
					return result;
			}
			if (PhidgetBLDCMotor_getLastPosition((PhidgetBLDCMotorHandle)ch) != position) {
				result = bridgeSendToChannel(ch, BP_POSITIONCHANGE, 1, "%l", position);
				if (result != EPHIDGET_OK)
					return result;
			}
			return (bridgeSendToChannel(ch, BP_DUTYCYCLECHANGE, 1, "%g", dutyCycle));
		case VINT_PACKET_TYPE_BLDCMOTOR_STALLDETECTED:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_MOTORSTALL, "Dangerous motor stall detected. Velocity has been reduced in order to prevent damage."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_DCC1100_BLDCMOTOR_120:
	...
	case PHIDCHUID_DCC1100_MOTORPOSITIONCONTROLLER_100:
		...
		case VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_MOTORSTATUSUPDATE:
			...
			return (bridgeSendToChannel(ch, BP_DUTYCYCLECHANGE, 1, "%g", dutyCycle));
		case VINT_PACKET_TYPE_BLDCMOTOR_STALLDETECTED:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_MOTORSTALL, "Dangerous motor stall detected. Motor has been disengaged in order to prevent damage."));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_DCC1100_MOTORPOSITIONCONTROLLER_120:
...
#if PHIDUID_DCC1120_SUPPORTED
static PhidgetReturnCode
handleDCC11X0_Error(PhidgetChannelHandle ch, uint8_t code, uint8_t subCode) {
	switch (code) {
	case 1:
		return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_FAILSAFE, "Failsafe procedure initiated. You must close and re-open your controller."));
	case 2:
		return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_ESTOP, "External stop procedure initiated. You must close and re-open your controller."));
	case 3:
		switch (subCode) {
		case 1:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Motor initialization failed. Suspected short circut. Fix connection and re-open controller to try again."));
		case 2:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Motor initialization failed. Probable open circuit or purely resistive load. Fix connection and re-open controller to try again."));
		case 3:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Motor initialization failed. Inductance measured too high for the controller. Fix connection and re-open controller to try again."));
		case 4:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Motor initialization failed. Inductance measured too low for the controller. Close the Motor Controller and re-open it to try again."));
		case 5:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Motor initialization failed. Verify motor is not moving and re-open controller to try again."));
		case 6:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Motor initialization failed. Coil mismatch. Possible open circuit or bad motor. Fix connection and re-open controller to try again."));
		case 7:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Hall sensor error. The Hall sensor is likely damaged or unplugged. Fix connection and re-open controller to try again."));
		default:
			MOS_PANIC("Unexpected error sub-code");
		}
	case 4:
		return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_ENERGYDUMP, "Motor Controller forced to save itself from excess regenerative power. We recommend adding an appropriate PowerGuard Phidget to your application."));
	case 5:
		return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OVERTEMP, "Motor Controller experience an overtemperature condition. Consider adding better airflow to your system."));
	default:
		MOS_PANIC("Unexpected error code");
	}
}
#endif /* PHIDUID_DCC1120_SUPPORTED */
...
#if PHIDUID_DCC1120_SUPPORTED
static PhidgetReturnCode
sendDCC1120(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[8] = {0};
	...
	switch (ch->UCD->uid) {
	#if PHIDUID_DCC1110_SUPPORTED
	case PHIDCHUID_DCC1110_CURRENTINPUT_100:
	#endif // PHIDUID_DCC1110_SUPPORTED
	case PHIDCHUID_DCC1120_CURRENTINPUT_100:
	#if PHIDUID_DCC1130_SUPPORTED
	case PHIDCHUID_DCC1130_CURRENTINPUT_100:
	#endif // PHIDUID_DCC1130_SUPPORTED
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
	...
	#if PHIDUID_DCC1110_SUPPORTED
	case PHIDCHUID_DCC1110_VOLTAGEINPUT_100:
	#endif // PHIDUID_DCC1110_SUPPORTED
	case PHIDCHUID_DCC1120_VOLTAGEINPUT_100:
	#if PHIDUID_DCC1130_SUPPORTED
	case PHIDCHUID_DCC1130_VOLTAGEINPUT_100:
	#endif // PHIDUID_DCC1130_SUPPORTED
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
	...
	#if PHIDUID_DCC1110_SUPPORTED
	case PHIDCHUID_DCC1110_TEMPERATURESENSOR_IC_100:
	#endif // PHIDUID_DCC1110_SUPPORTED
	case PHIDCHUID_DCC1120_TEMPERATURESENSOR_IC_100:
	#if PHIDUID_DCC1130_SUPPORTED
	case PHIDCHUID_DCC1130_TEMPERATURESENSOR_IC_100:
	#endif // PHIDUID_DCC1130_SUPPORTED
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
	...
	#if PHIDUID_DCC1110_SUPPORTED
	case PHIDCHUID_DCC1110_ENCODER_100:
	#endif // PHIDUID_DCC1110_SUPPORTED
	case PHIDCHUID_DCC1120_ENCODER_100:
	#if PHIDUID_DCC1130_SUPPORTED
	case PHIDCHUID_DCC1130_ENCODER_100:
	#endif // PHIDUID_DCC1130_SUPPORTED
		switch (bp->vpkt) {
		case BP_SETDATAINTERVAL:
			pack16(buf, HANDLE_DATAINTERVAL_PKT(bp, 1));
	...
	case PHIDCHUID_DCC1130_MOTORPOSITIONCONTROLLER_100:
		...
		case BP_POSITIONTYPE:
			buf[0] = (uint8_t)getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_BLDCMOTOR_SETPOSITIONTYPE, buf, 1));
		case BP_SETENABLEEXPECTEDPOSITION:
			buf[0] = getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_ENABLEEXPECTEDPOS, buf, 1));
		case BP_SETFAILSAFEBRAKINGDUTYCYCLE:
...
static PhidgetReturnCode
recvDCC1120(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	double temperature, voltage, current, dutyCycle, brakingDutyCycle;
	double currentLimit, surgeCurrentLimit, activeCurrentLimit;
	int indexPosition;
	int64_t position;
	...
	case PHIDCHUID_DCC1130_CURRENTINPUT_100:
		...
		case VINT_PACKET_TYPE_CURRENTINPUT_CURRENTCHANGE:
			current = (double)(unpackfloat(buf));
			current = round_double((current), 4);
			result = EPHIDGET_OK;
			if (len > 4) {
				if (buf[4] == VINT_PACKET_ERRORBYTE_OUTOFRANGEHIGH)
					result = (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Current reading is above the valid range"));
				else if (buf[4] == VINT_PACKET_ERRORBYTE_OUTOFRANGELOW)
					result = (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGELOW, "Current reading is below the valid range"));
			}
			if (result != EPHIDGET_OK)
				return result;
			return (bridgeSendToChannel(ch, BP_CURRENTCHANGE, 1, "%g", current));
		case VINT_PACKET_TYPE_CURRENTINPUT_SATURATION:
			if (buf[0] == VINT_PACKET_ERRORBYTE_OUTOFRANGEHIGH)
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Current reading is above the valid range"));
			else if (buf[0] == VINT_PACKET_ERRORBYTE_OUTOFRANGELOW)
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGELOW, "Current reading is below the valid range"));
			else if (buf[0] == VINT_PACKET_ERRORBYTE_OUTOFRANGE) {
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGE, "Hall sensor unplugged, current reading unknown"));
			}
			break;
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
	...
	case PHIDCHUID_DCC1130_VOLTAGEINPUT_100:
		...
		case VINT_PACKET_TYPE_VOLTAGEINPUT_VOLTAGECHANGE:
			if (len > 4) {
				result = (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGEHIGH, "Source voltage is above the measurement range."));
				if (result != EPHIDGET_OK)
					return result;
			}
			voltage = unpackfloat(buf);
			return (bridgeSendToChannel(ch, BP_VOLTAGECHANGE, 1, "%g", voltage));
		case VINT_PACKET_TYPE_VOLTAGEINPUT_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGEHIGH, "Source voltage is above the measurement range."));
			break;
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
			return bridgeSendToChannel(ch, BP_DATAINTERVALCHANGE, 1, "%u", interval);
	...
	case PHIDCHUID_DCC1130_TEMPERATURESENSOR_IC_100:
		...
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_TEMPERATURECHANGE:
			...
			if (len > 4) {
				if (buf[4] == VINT_PACKET_ERRORBYTE_OUTOFRANGEHIGH)
					result = (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Temperature reading is above the valid range"));
				else if (buf[4] == VINT_PACKET_ERRORBYTE_OUTOFRANGELOW)
					result = (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGELOW, "Temperature reading is below the valid range"));
			}
			if (result != EPHIDGET_OK)
				return result;
			return (bridgeSendToChannel(ch, BP_TEMPERATURECHANGE, 1, "%g", temperature));
		case VINT_PACKET_TYPE_TEMPERATURESENSOR_SATURATION:
			if (buf[0] == VINT_PACKET_ERRORBYTE_OUTOFRANGEHIGH)
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Temperature reading is above the valid range"));
			else if (buf[0] == VINT_PACKET_ERRORBYTE_OUTOFRANGELOW)
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGELOW, "Temperature reading is below the valid range"));
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
			return bridgeSendToChannel(ch, BP_DATAINTERVALCHANGE, 1, "%u", interval);
		default:
	...
	case PHIDCHUID_DCC1130_BLDCMOTOR_100:
		...
		case VINT_PACKET_TYPE_DCMOTOR_MOTORSTATUSUPDATE:
			brakingDutyCycle = (unpackfloat(buf + 4));
			brakingDutyCycle = round_double(brakingDutyCycle, 3);
			dutyCycle = unpackfloat(buf);
			dutyCycle = round_double(dutyCycle, 3);
			position = ((int64_t)unpack64(buf + 8));
			activeCurrentLimit = unpackfloat(buf + 16);
			activeCurrentLimit = round_double(activeCurrentLimit, 1);
			sendInd = (len > 20);
			if (sendInd) {
				inductance = unpackfloat(buf + 20);
				inductance = round_double(inductance, 9);
			} else {
				inductance = PUNK_DBL;
			}
			result = (bridgeSendToChannel(ch, BP_ACTIVECURRENTLIMITCHANGE, 1, "%g", activeCurrentLimit));
			if (result != EPHIDGET_OK)
				return result;
			result = bridgeSendToChannel(ch, BP_BRAKINGSTRENGTHCHANGE, 1, "%g", brakingDutyCycle);
			if (result != EPHIDGET_OK)
			...
			if (result != EPHIDGET_OK)
				return result;
			result = (bridgeSendToChannel(ch, BP_DUTYCYCLECHANGE, 1, "%g", dutyCycle));
			if (result != EPHIDGET_OK)
				return result;
			if (sendInd)
				return (bridgeSendToChannel(ch, BP_INDUCTANCECHANGE, 1, "%g", inductance));
			else
				return EPHIDGET_OK;
		case VINT_PACKET_TYPE_MOTORCONTROLLER_FAILSAFE:
			return handleDCC11X0_Error(ch, buf[0], buf[1]);
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
			return bridgeSendToChannel(ch, BP_DATAINTERVALCHANGE, 1, "%u", interval);
		case VINT_PACKET_TYPE_DCMOTOR_CURRENTLIMITRESPONSE:
			currentLimit = unpackfloat(buf);
			surgeCurrentLimit = unpackfloat(buf + 4);
			if (currentLimit > 0) {
				result = bridgeSendToChannel(ch, BP_CURRENTLIMITCHANGE, 1, "%g", currentLimit);
				if (result != EPHIDGET_OK)
					return result;
			}
			if (surgeCurrentLimit > 0) {
				result = bridgeSendToChannel(ch, BP_SURGECURRENTLIMITCHANGE, 1, "%g", surgeCurrentLimit);
				if (result != EPHIDGET_OK)
					return result;
			}
			return EPHIDGET_OK;
	...
	case PHIDCHUID_DCC1130_MOTORPOSITIONCONTROLLER_100:
		...
		case VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_MOTORSTATUSUPDATE:
			...
			dutyCycle = round_double(dutyCycle, 3);
			position = ((int64_t)unpack64(buf + 4));
			activeCurrentLimit = unpackfloat(buf + 12);
			activeCurrentLimit = round_double(activeCurrentLimit, 1);
			sendExpected = (len == 24 || len == 28);
			if (sendExpected) {
				expectedPosition = unpack64(buf + 16 + dataOffset);
				dataOffset += 8;
			} else {
				expectedPosition = PUNK_INT64;
			}
			sendInd = (len == 20 || len == 28);
			if (sendInd) {
				inductance = unpackfloat(buf + dataOffset + 16);
				inductance = round_double(inductance, 9);
				dataOffset += 4;
			} else {
				inductance = PUNK_DBL;
			...
			if (sendExpected) {
				result = bridgeSendToChannel(ch, BP_EXPECTEDPOSITIONCHANGE, 1, "%l", expectedPosition);
				if (result != EPHIDGET_OK)
					return result;
			}
			result = (bridgeSendToChannel(ch, BP_ACTIVECURRENTLIMITCHANGE, 1, "%g", activeCurrentLimit));
	...
	case PHIDCHUID_DCC1130_MOTORVELOCITYCONTROLLER_100:
		...
		case VINT_PACKET_TYPE_MOTORPOSITIONCONTROLLER_MOTORSTATUSUPDATE:
			...
			dutyCycle = unpackfloat(buf);
			dutyCycle = round_double(dutyCycle, 3);
			velocity = unpackfloat(buf + 4);
			velocity = round_double(velocity, 3);
			activeCurrentLimit = unpackfloat(buf + 8);
			activeCurrentLimit = round_double(activeCurrentLimit, 1);
			if (len > 12) {
				sendExpected = (buf[12] & 1) != 0;
				sendInd = (buf[12] & 2) != 0;
				dataOffset++;
			} else {
				sendInd = 0;
			...
			if (sendExpected) {
				expectedVelocity = unpackfloat(buf + 12 + dataOffset);
				dataOffset += 4;
			} else {
				expectedVelocity = PUNK_DBL;
			}
			if (sendInd) {
				inductance = unpackfloat(buf + 12 + dataOffset);
				inductance = round_double(inductance, 9);
				dataOffset += 4;
			} else {
				inductance = PUNK_DBL;
			...
			if (result != EPHIDGET_OK)
				return result;
			if (sendExpected) {
				result = bridgeSendToChannel(ch, BP_EXPECTEDVELOCITYCHANGE, 1, "%g", expectedVelocity);
				if (result != EPHIDGET_OK)
					return result;
			}
			result = bridgeSendToChannel(ch, BP_VELOCITYCHANGE, 1, "%g", velocity);
			if (result != EPHIDGET_OK)
				return result;
			result = (bridgeSendToChannel(ch, BP_DUTYCYCLECHANGE, 1, "%g", dutyCycle));
			if (result != EPHIDGET_OK)
...
#if PHIDUID_DCC1110_SUPPORTED
static PhidgetReturnCode
sendDCC1110(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendDCC1120(ch, bp);
}

static PhidgetReturnCode
recvDCC1110(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recvDCC1120(ch, buf, len);
}
#endif /* PHIDUID_DCC1110_SUPPORTED */

#if PHIDUID_DCC1130_SUPPORTED
static PhidgetReturnCode
sendDCC1130(PhidgetChannelHandle ch, BridgePacket *bp) {
	return sendDCC1120(ch, bp);
}

static PhidgetReturnCode
recvDCC1130(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recvDCC1120(ch, buf, len);
}
#endif /* PHIDUID_DCC1130_SUPPORTED */
...
recvDCC1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DCC1000_VOLTAGERATIOINPUT_100:
		...
		case VINT_PACKET_TYPE_VOLTAGERATIOINPUT_VOLTAGERATIOCHANGE:
			...
			return (bridgeSendToChannel(ch, BP_VOLTAGERATIOCHANGE, 1, "%g", voltageRatio));
...
#if PHIDUID_DCC1005_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendDCC1005_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvDCC1005_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_DCC1005_OBSOLETE_SUPPORTED */

#if PHIDUID_DCC1020_SUPPORTED
static PhidgetReturnCode
handleDCC10X0_Error(PhidgetChannelHandle ch, uint8_t code, uint8_t subCode) {
	switch (code) {
	case 1:
		return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_FAILSAFE, "Failsafe procedure initiated."));
	case 2:
		return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_ESTOP, "External stop procedure initiated."));
	case 3:
		switch (subCode) {
		case 1:
			return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Motor initialization failed. Suspected short circut. Fix connection and re-open controller to try again."));
...
#if PHIDUID_DCC1020_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendDCC1020_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvDCC1020_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {

	logwarn("Ignoring incoming packet from obsolete firmware");
	return (EPHIDGET_OK);
}
#endif /* PHIDUID_DCC1020_OBSOLETE_SUPPORTED */

#if PHIDUID_DCC1020_SUPPORTED
static PhidgetReturnCode
recvDCC1020(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	double temperature, voltage, current, dutyCycle, brakingDutyCycle;
	double currentLimit, surgeCurrentLimit, activeCurrentLimit;
	int indexPosition;
	int64_t position;
	...
	case PHIDCHUID_DCC1030_DCMOTOR_100:
		...
		case VINT_PACKET_TYPE_DCMOTOR_MOTORSTATUSUPDATE:
			...
			if (sendInd)
				...
				return EPHIDGET_OK;
		case VINT_PACKET_TYPE_MOTORCONTROLLER_FAILSAFE:
			return handleDCC10X0_Error(ch, buf[0], buf[1]);
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpack16(buf);
			return bridgeSendToChannel(ch, BP_DATAINTERVALCHANGE, 1, "%u", interval);
		case VINT_PACKET_TYPE_DCMOTOR_CURRENTLIMITRESPONSE:
...
#if PHIDUID_DCC1010_SUPPORTED
static PhidgetReturnCode
recvDCC1010(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recvDCC1020(ch, buf, len);
}
#endif /* PHIDUID_DCC1010_SUPPORTED */

#if PHIDUID_DCC1030_SUPPORTED
static PhidgetReturnCode
recvDCC1030(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	return recvDCC1020(ch, buf, len);
}
#endif /* PHIDUID_DCC1030_SUPPORTED */
...
#if PHIDUID_DAQ1501_SUPPORTED
static PhidgetReturnCode
recvDAQ1501(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	const int DAQ1501_OUTOFRANGEHIGH_FLAG = 0x01;
	const int DAQ1501_OUTOFRANGELOW_FLAG = 0x02;
	...
	switch (ch->UCD->uid) {
	case PHIDCHUID_DAQ1501_VOLTAGERATIOINPUT_100:
		switch (pkt) {
		case VINT_PACKET_TYPE_VOLTAGERATIOINPUT_VOLTAGERATIOCHANGE:
			for (queueIndex = 0; queueIndex < (len / 4); queueIndex++) {
				voltageRatio = (int32_t)unpack32(&buf[queueIndex * 4]) / (0x80000000u - 1.0);
				res = (bridgeSendToChannel(ch, BP_VOLTAGERATIOCHANGE, 1, "%g", voltageRatio));
				if (res != EPHIDGET_OK)
					return res;
				if ((len % 4) != 0) {
					if ((buf[len - 1] >> (2 * queueIndex)) & DAQ1501_OUTOFRANGEHIGH_FLAG) {
						res = (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGEHIGH, "Sensor value is above the valid range for its gain setting."));
						if (res != EPHIDGET_OK)
							return res;
					} else if ((buf[len - 1] >> (2 * queueIndex)) & DAQ1501_OUTOFRANGELOW_FLAG) {
						res = (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_OUTOFRANGELOW, "Sensor value is below the valid range for its gain setting."));
						if (res != EPHIDGET_OK)
							return res;
					}
				}
			...
			return res;
		case VINT_PACKET_TYPE_DATAINTERVAL_RESP:
			interval = unpackfloat(buf);
			integerInterval = round((interval > 0) ? interval : 1);
			res = bridgeSendToChannel(ch, BP_DATAINTERVALCHANGE, 2, "%u%g", integerInterval, interval);
			return res;
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
static PhidgetReturnCode
sendDAQ1501(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[4];
	assert(ch);
	assert(bp);
	...
	case PHIDCHUID_DAQ1501_VOLTAGERATIOINPUT_100:
		...
		case BP_SETDATAINTERVAL:
			if (bp->entrycnt > 1)
				packfloat(buf, getBridgePacketDouble(bp, 1));
			else
				packfloat(buf, (float)getBridgePacketUInt32(bp, 0));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_SAMPLED_SETDATAINTERVAL, buf, 4));
		case BP_SETCHANGETRIGGER:
			pack32(buf, (uint32_t)(getBridgePacketDouble(bp, 0) * 0x80000000u));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_VOLTAGERATIOINPUT_SETVOLTAGERATIOCHANGETRIGGER, buf, 4));
		case BP_SETBRIDGEGAIN:
...
#endif // PHIDUID_VCP1100_1_OLDPROTO_SUPPORTED
...
#if PHIDUID_VCP1100_1_OLDPROTO_SUPPORTED
static PhidgetReturnCode
...
sendDAQ1400(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DAQ1400_FREQUENCYCOUNTER_120:
		...
		case BP_SETPOWERSUPPLY:
			buf[0] = getBridgePacketInt32(bp, 0);
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_ISA_SETPOWERSUPPLY, buf, 1);
			// Need to update the other channels because power supply is board-wide
			if (ret == EPHIDGET_OK) {
				phid->powerSupply = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_POWERSUPPLYCHANGE, 1, "%d", phid->powerSupply);
			}
			return (ret);
		case BP_SETINPUTMODE:
			buf[0] = getBridgePacketInt32(bp, 0);
			ret = sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_ISA_SETINPUTMODE, buf, 1);
			// Need to update the other channels because input mode is board-wide
			if (ret == EPHIDGET_OK) {
				phid->inputMode = getBridgePacketInt32(bp, 0);
				bridgeSendToChannelSiblings(ch, BP_INPUTMODECHANGE, 1, "%d", phid->inputMode);
			}
			return (ret);
		default:
			MOS_PANIC("Unexpected packet type");
...
recvDAQ1400(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DAQ1400_FREQUENCYCOUNTER_120:
		...
		case VINT_PACKET_TYPE_FREQUENCYCOUNTER_COUNTCHANGE:
			...
			return (bridgeSendToChannel(ch, BP_FREQUENCYDATA, 3, "%g%u%g", ticks, counts, ticksAtLastCount));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvDAQ1301(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DAQ1301_DIGITALINPUT_100:
		switch (pkt) {
		case VINT_PACKET_TYPE_DIGITALINPUT_STATECHANGE:
			return (bridgeSendToChannel(ch, BP_STATECHANGE, 1, "%d", (buf[0] & 0x1)));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvOUT1002(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_OUT1002_VOLTAGEOUTPUT_100:
		...
		case VINT_PACKET_TYPE_VOLTAGEOUTPUT_VOLTAGEERROR:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_VOLTAGEERROR, "Voltage Error Detected"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	case PHIDCHUID_OUT1002_VOLTAGEOUTPUT_110:
...
#if PHIDUID_CURLOOP_OBSOLETE_SUPPORTED
static PhidgetReturnCode
sendCURLOOP_OBSOLETE(PhidgetChannelHandle ch, BridgePacket *bp) {
	MOS_PANIC("Obsolete Firmware");
}

static PhidgetReturnCode
recvCURLOOP_OBSOLETE(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
...
recvCURLOOP(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_CURLOOP_CURRENTOUTPUT_100:
		...
		case VINT_PACKET_TYPE_CURRENTOUTPUT_CURRENTERROR:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_BADPOWER, "Current Error Detected"));
		default:
			MOS_PANIC("Unexpected packet type");
		}
...
recvDAQ1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_DAQ1000_VOLTAGERATIOINPUT_110:
		...
		case VINT_PACKET_TYPE_VOLTAGERATIOINPUT_VOLTAGERATIOCHANGE:
			...
			return (bridgeSendToChannel(ch, BP_VOLTAGERATIOCHANGE, 1, "%g", voltage));
		case VINT_PACKET_TYPE_VOLTAGERATIOINPUT_SATURATION:
			return (PhidgetChannel_sendErrorEventThrottled(ch, EEPHIDGET_SATURATION, "Saturation Detected."));
		default:
			MOS_PANIC("Unexpected packet type");
...
recvADP1000(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_ADP1000_PHSENSOR_100:
		...
		case VINT_PACKET_TYPE_PHSENSOR_PHCHANGE:
			...
			return (bridgeSendToChannel(ch, BP_PHCHANGE, 1, "%g", pH));
...
#if PHIDUID_VOLTAGERATIOINPUT_PORT_V2_SUPPORTED
static PhidgetReturnCode
sendVOLTAGERATIOINPUT_PORT_V2(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[4];
...
static PhidgetReturnCode
recvVOLTAGERATIOINPUT_PORT_V2(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	double voltageRatio;
	int pkt;
		...
		case VINT_PACKET_TYPE_VOLTAGERATIOINPUT_VOLTAGERATIOCHANGE:
			if (len == 5) {
				...
				case VINT_PACKET_ERRORBYTE_OUTOFRANGEHIGH:
					PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Voltage Ratio is too high to be accurately measured");
					break;
				default:
					MOS_PANIC("Unexpected error event code");
				}
			}
			voltageRatio = round_double(unpackfloat(buf), 6);
			return (bridgeSendToChannel(ch, BP_VOLTAGERATIOCHANGE, 1, "%g", voltageRatio));

		case VINT_PACKET_TYPE_VOLTAGERATIOINPUT_SATURATION:
			switch (buf[0]) {
			case VINT_PACKET_ERRORBYTE_OUTOFRANGEHIGH:
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Voltage Ratio is too high to be accurately measured"));
			default:
				MOS_PANIC("Unexpected error event code");
			}
...
#if PHIDUID_VOLTAGEINPUT_PORT_V2_SUPPORTED
static PhidgetReturnCode
sendVOLTAGEINPUT_PORT_V2(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[4];
...
static PhidgetReturnCode
recvVOLTAGEINPUT_PORT_V2(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	double voltage;
	int pkt;
	...
	case PHIDCHUID_HUB_VOLTAGEINPUT_200:
		...
		case VINT_PACKET_TYPE_VOLTAGEINPUT_VOLTAGECHANGE:
			if (len == 5) {
				switch (buf[4]) {
				case VINT_PACKET_ERRORBYTE_OUTOFRANGEHIGH:
					PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Voltage is too high to be accurately measured");
					break;
				default:
					MOS_PANIC("Unexpected error event code");
				}
		...
		case VINT_PACKET_TYPE_VOLTAGEINPUT_SATURATION:
			switch (buf[0]) {
			case VINT_PACKET_ERRORBYTE_OUTOFRANGEHIGH:
				return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_OUTOFRANGEHIGH, "Voltage is too high to be accurately measured"));
			default:
				MOS_PANIC("Unexpected error event code");
			}
...
recvDIGITALINPUT_PORT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	...
	case PHIDCHUID_HUB_DIGITALINPUT_100:
		switch (pkt) {
		case VINT_PACKET_TYPE_DIGITALINPUT_STATECHANGE:
			return (bridgeSendToChannel(ch, BP_STATECHANGE, 1, "%d", 1));
		case VINT_PACKET_TYPE_DIGITALINPUT_STATECHANGE2:
			return (bridgeSendToChannel(ch, BP_STATECHANGE, 1, "%d", 0));
		default:
			MOS_PANIC("Unexpected packet type");
		}
