...
static void
dispatchErrorNotify(PhidgetHandle phid, int throwError, Phidget_LogLevel lvl, const char *fmt, ...) {
	...
	if (throwError) {
		...
		if (_ch_ && _ch_->Error) {
			mos_vsnprintf(_errbuf_, sizeof(_errbuf_), fmt, va);
			_ch_->Error((PhidgetHandle)_ch_, _ch_->ErrorCtx, EEPHIDGET_DISPATCH, _errbuf_);
		}
	}
	PhidgetLog_logv(__FILE__, __LINE__, __func__, NULL, lvl, fmt, va);
	va_end(va);
}
...
insertDispatchEntry(PhidgetHandle phid, DispatchEntryHandle de) {
	...
	if (de->type > DE_THROW_AWAY && dph->count >= soft) {
		PhidgetUnlock(phid);
		dispatchErrorNotify(phid, dph->softErrorThrown ? PFALSE : PTRUE, PHIDGET_LOG_WARNING,
							"%" PRIphid ": Event queue is full; dropping event(s). Make sure data event handlers are fast and non-blocking, or reduce data rate.", phid);
		returnDispatchEntry(de);
		dph->softErrorThrown = 1;
		/*
	...
	} else if (dph->softErrorThrown && dph->count < (soft - 5)) {
		// Don't clear the soft error flag until we have >= 5 free space in the dispatch queue
		dph->softErrorThrown = 0;
	}
	...
	if (dph->count > hard) {
		PhidgetUnlock(phid);
		if (OUTBOUND(de))
			dispatchErrorNotify(phid, PTRUE, PHIDGET_LOG_ERROR,
								"%" PRIphid ": Event queue is full (hard); dropping entry (type=%d). Make sure data event handlers are fast and non-blocking, or reduce data rate.", phid, de->type);
		else
			dispatchErrorNotify(phid, PTRUE, PHIDGET_LOG_ERROR,
								"%" PRIphid ": Command queue is full; dropping entry (type=%d). If sending commands from multiple threads or using async sets, make sure that total pending commands is less than %d.", phid, de->type, hard);
		returnDispatchEntry(de);
		return (EPHIDGET_NOSPC);
	}
...
dispatchUserRequest(PhidgetChannelHandle ch, BridgePacket *bp, Phidget_AsyncCallback cb, void *ctx) {
	...
	if (cb == NULL) {
		...
		while (de->flags & DISPATCHENTRY_WAITING) {
			 ...
			 */
			if (mos_gettime_usec() - tm > 100000000) { // 100 seconds (needed for 64k vint device firmware upgrade)
...
dispatchChannelBridgePacket(PhidgetChannelHandle channel, BridgePacket *bp) {
	...
	// NOTE: We got a non-error bridge packet from the local device - clear the error to ensure that the next error we see is reported right away.
	if (bp->vpkt != BP_ERROREVENT && !bridgePacketIsFromNet(bp))
	...
	if (channel) {
		...
		case DE_CHANNEL_SETSTATUS:
			...
			MTAILQ_FOREACH(cnc, &channel->netconns, link) {
				...
				if (res != EPHIDGET_OK) {
					...
					sendSimpleReply(cnc->nc, repseq, res, "getStatus() failure.");
					logerr("%" PRIphid ": failed to get status", channel);
					device = getParent(channel);
					closeServerChannel(PHIDID(device), channel->uniqueIndex, channel->openInfo->openCloseReset, cnc->nc);
		...
		case DE_CHANNEL_ATTACH: {
			...
			if (!isNetworkPhidget(channel)) {
				if (channel->openInfo->openCloseReset || isVintChannel(channel)) {
					res = createBridgePacket(&bp, BP_OPENRESET, 0, NULL);
					if (res == EPHIDGET_OK) {
				...
				// Signals that RESET (and set defaults) is finished, we can start accepting channel packets
				PhidgetSetFlags(channel, PHIDGET_INITIALIZED_FLAG);
			}
			...
			// Only for local attach
			if (!isNetworkPhidget(channel)) {
				res = createBridgePacket(&bp, BP_ENABLE, 0, NULL);
				if (res == EPHIDGET_OK) {
					res = DEVBRIDGEINPUT(channel, bp);
					destroyBridgePacket(&bp);
				}
		...
		case DE_USERREQ:
			/*
			 * NOTE: These are bridge packets from channel->device
			 */
			...
			if (res == EPHIDGET_OK) {
				if (isNetworkPhidget(channel)) {
					...
					// This ends up in DE_SERVERBRIDGEPACKET on the server
		...
		case DE_CHANNEL_BRIDGEPKT:
			/*
			 * NOTE: These are bridge packets from device->channel
			 *
		...
		case DE_CLIENTBRIDGEPACKET:
			if (bridgePacketIsEvent(de->de_bpe.bp)) {
				 ...
				 * about the result code as the server isn't going to wait to hear about it.
				 */
		...
		case DE_SERVERBRIDGEPACKET:
			if (bridgePacketIsEvent(de->de_bpe.bp)) {
				// NOTE: We shouldn't ever see this, as clients don't send events to the server..
				res = channelDeliverBridgePacket(channel, de->de_bpe.bp, de->de_bpe.nc, de->de_bpe.forward);
			} else {
				mosiop_t iop;
				/*
				...
				// NOTE: this calls channel bridgeInput, and then also forwards the packet to other clients
				res = channelDeliverBridgePacket(channel, de->de_bpe.bp, de->de_bpe.nc, de->de_bpe.forward);
				if (res != EPHIDGET_OK) {
					char *err;
...
void startDispatch(PhidgetHandle phid) {
	PhidgetManagerHandle manager;
	PhidgetChannelHandle channel;
	PhidgetDeviceHandle device;
...
void stopDispatch(PhidgetHandle phid, int sendEvents) {
	PhidgetChannelHandle channel;
	PhidgetDeviceHandle device;
...
void clearPhidgetDispatch(PhidgetHandle phid) {
	PhidgetChannelHandle channel;
	DispatchEntryHandle de;
	DispatchHandle dph;
...
void freeDispatchHandle(PhidgetHandle phid) {
