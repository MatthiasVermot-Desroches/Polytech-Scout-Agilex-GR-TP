...
Phidget_strerrordetail(PhidgetReturnCode code) {
	...
	case EPHIDGET_BADPOWER:
		return ("The power supply of your device is outside the acceptable range to allow operation.");
	case EPHIDGET_POWERCYCLE:
		return ("Something has caused your device to decide it needs to be powered off and on to resume operation.");
	case EPHIDGET_HALLSENSOR:
		return ("The hall sensor on your BLDC Motor Controller is Improperly Connected");
	case EPHIDGET_BADCURRENT:
		return ("Current sensor offset outside acceptable bounds. Move the sensor aways from magnetic fields and try again.");
	default:
		return ("Invalid return code");
	}
}
