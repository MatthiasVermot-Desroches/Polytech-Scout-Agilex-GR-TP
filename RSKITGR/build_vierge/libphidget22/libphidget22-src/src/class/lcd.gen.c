...
_setDefaults(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_1219_TEXTLCD_000:
		break;
	case PHIDCHUID_LCD1100_LCD_100:
		ret = bridgeSendToDevice(phid, BP_SETCONTRAST, NULL, NULL, 1, "%g", ch->contrast);
		if (ret != EPHIDGET_OK)
			break;
		ret = bridgeSendToDevice(phid, BP_SETFRAMEBUFFER, NULL, NULL, 1, "%d", ch->frameBuffer);
		if (ret != EPHIDGET_OK)
			break;
		break;
	case PHIDCHUID_LCD1100_LCD_200:
...
PhidgetLCD_clear_async(PhidgetLCDHandle ch, Phidget_AsyncCallback fptr, void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_CLEAR, fptr, ctx, 0, NULL);
...
PhidgetLCD_copy(PhidgetLCDHandle ch, int sourceFramebuffer, int destFramebuffer, int sourceX1,
  int sourceY1, int sourceX2, int sourceY2, int destX, int destY, int inverted) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_COPY, NULL, NULL, 9, "%d%d%d%d%d%d%d%d%d",
...
PhidgetLCD_copy_async(PhidgetLCDHandle ch, int sourceFramebuffer, int destFramebuffer, int sourceX1,
  int sourceY1, int sourceX2, int sourceY2, int destX, int destY, int inverted, Phidget_AsyncCallback fptr, void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_COPY, fptr, ctx, 9, "%d%d%d%d%d%d%d%d%d",
	  sourceFramebuffer, destFramebuffer, sourceX1, sourceY1, sourceX2, sourceY2, destX, destY, inverted);
...
PhidgetLCD_drawLine(PhidgetLCDHandle ch, int x1, int y1, int x2, int y2) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_DRAWLINE, NULL, NULL, 4, "%d%d%d%d", x1, y1,
	  x2, y2); 
}
...
PhidgetLCD_drawLine_async(PhidgetLCDHandle ch, int x1, int y1, int x2, int y2, Phidget_AsyncCallback fptr,
  void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_DRAWLINE, fptr, ctx, 4, "%d%d%d%d", x1, y1, x2,
	  y2);
...
PhidgetLCD_drawPixel(PhidgetLCDHandle ch, int x, int y, PhidgetLCD_PixelState pixelState) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_DRAWPIXEL, NULL, NULL, 3, "%d%d%d", x, y,
	  pixelState); 
}
...
PhidgetLCD_drawPixel_async(PhidgetLCDHandle ch, int x, int y, PhidgetLCD_PixelState pixelState,
  Phidget_AsyncCallback fptr, void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_DRAWPIXEL, fptr, ctx, 3, "%d%d%d", x, y,
	  pixelState);
...
PhidgetLCD_drawRect(PhidgetLCDHandle ch, int x1, int y1, int x2, int y2, int filled, int inverted) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_DRAWRECT, NULL, NULL, 6, "%d%d%d%d%d%d", x1,
	  y1, x2, y2, filled, inverted); 
}
...
PhidgetLCD_drawRect_async(PhidgetLCDHandle ch, int x1, int y1, int x2, int y2, int filled, int inverted,
  Phidget_AsyncCallback fptr, void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_DRAWRECT, fptr, ctx, 6, "%d%d%d%d%d%d", x1, y1,
...
PhidgetLCD_flush(PhidgetLCDHandle ch) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_FLUSH, NULL, NULL, 0, NULL); 
}
...
PhidgetLCD_flush_async(PhidgetLCDHandle ch, Phidget_AsyncCallback fptr, void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_FLUSH, fptr, ctx, 0, NULL);
...
PhidgetLCD_setFontSize(PhidgetLCDHandle ch, PhidgetLCD_Font font, int width, int height) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETFONTSIZE, NULL, NULL, 3, "%d%d%d", font,
	  width, height); 
}
...
PhidgetLCD_initialize(PhidgetLCDHandle ch) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_INITIALIZE, NULL, NULL, 0, NULL); 
}
...
PhidgetLCD_saveFrameBuffer(PhidgetLCDHandle ch, int frameBuffer) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SAVEFRAMEBUFFER, NULL, NULL, 1, "%d",
	  frameBuffer); 
}
...
PhidgetLCD_saveFrameBuffer_async(PhidgetLCDHandle ch, int frameBuffer, Phidget_AsyncCallback fptr,
  void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SAVEFRAMEBUFFER, fptr, ctx, 1, "%d",
	  frameBuffer);
...
PhidgetLCD_writeText(PhidgetLCDHandle ch, PhidgetLCD_Font font, int xPosition, int yPosition,
  const char *text) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_WRITETEXT, NULL, NULL, 4, "%d%d%d%s", font,
...
PhidgetLCD_writeText_async(PhidgetLCDHandle ch, PhidgetLCD_Font font, int xPosition, int yPosition,
  const char *text, Phidget_AsyncCallback fptr, void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_WRITETEXT, fptr, ctx, 4, "%d%d%d%s", font,
	  xPosition, yPosition, text);
...
PhidgetLCD_setBacklight(PhidgetLCDHandle ch, double backlight) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETBACKLIGHT, NULL, NULL, 1, "%g",
	  backlight));
}
...
PhidgetLCD_setContrast(PhidgetLCDHandle ch, double contrast) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETCONTRAST, NULL, NULL, 1, "%g",
	  contrast));
}
...
PhidgetLCD_setCursorBlink(PhidgetLCDHandle ch, int cursorBlink) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETCURSORBLINK, NULL, NULL, 1, "%d",
	  cursorBlink));
}
...
PhidgetLCD_setCursorOn(PhidgetLCDHandle ch, int cursorOn) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETCURSORON, NULL, NULL, 1, "%d",
	  cursorOn));
}
...
PhidgetLCD_setFrameBuffer(PhidgetLCDHandle ch, int frameBuffer) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETFRAMEBUFFER, NULL, NULL, 1, "%d",
	  frameBuffer));
}
...
PhidgetLCD_setFrameBuffer_async(PhidgetLCDHandle ch, int frameBuffer, Phidget_AsyncCallback fptr,
  void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETFRAMEBUFFER, fptr, ctx, 1, "%d",
	  frameBuffer);
	if (res != EPHIDGET_OK && fptr != NULL)
		fptr((PhidgetHandle)ch, ctx, res);
}
...
PhidgetLCD_setScreenSize(PhidgetLCDHandle ch, PhidgetLCD_ScreenSize screenSize) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETSCREENSIZE, NULL, NULL, 1, "%d",
	  screenSize));
}
...
PhidgetLCD_setSleeping(PhidgetLCDHandle ch, int sleeping) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETSLEEP, NULL, NULL, 1, "%d", sleeping));
}
