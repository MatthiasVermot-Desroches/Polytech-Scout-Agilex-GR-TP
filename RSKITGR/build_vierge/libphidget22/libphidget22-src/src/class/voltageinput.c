...
PhidgetVoltageInput_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_SETSENSORTYPE:
		...
		if (res == EPHIDGET_OK) {
			...
			case SENSOR_TYPE_MOT2002_MED:
			case SENSOR_TYPE_MOT2002_HIGH:
				createBridgePacket(&subbp, BP_SETDATAINTERVAL, 1, "%u", 200);
	...
	case BP_SETVOLTAGERANGE:
		...
		if (res == EPHIDGET_OK) {
			...
			if (bridgePacketIsFromNet(bp)) {
				FIRE_PROPERTYCHANGE(ch, "MinVoltage");
				FIRE_PROPERTYCHANGE(ch, "MaxVoltage");
			} else {
				// We are running in the context of a set(), so we have to dispatch the PROPERTYCHANGE events
				DISPATCH_PROPERTYCHANGE(ch, "MinVoltage");
				DISPATCH_PROPERTYCHANGE(ch, "MaxVoltage");
			}
		}
		break;
	default:
