/* Generated: Mon Mar 04 2024 17:00:44 GMT-0700 (Mountain Standard Time) */
...
_setDefaults(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_1058_PHADAPTER_100:
		...
		if (ret != EPHIDGET_OK)
			break;
		ret = bridgeSendToDevice(phid, BP_SETCHANGETRIGGER, NULL, NULL, 1, "%g", ch->PHChangeTrigger);
		if (ret != EPHIDGET_OK)
			break;
		ret = bridgeSendToDevice(phid, BP_SETCORRECTIONTEMPERATURE, NULL, NULL, 1, "%g",
		  ch->correctionTemperature);
		if (ret != EPHIDGET_OK)
			break;
		break;
...
PhidgetPHSensor_setCorrectionTemperature(PhidgetPHSensorHandle ch, double correctionTemperature) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETCORRECTIONTEMPERATURE, NULL, NULL, 1,
	  "%g", correctionTemperature));
}
...
PhidgetPHSensor_getPH(PhidgetPHSensorHandle ch, double *PH) {
	...
	*PH = ch->PH;
	if (ch->PH == (double)PUNK_DBL || isnan(ch->PH))
		return (PHID_RETURN(EPHIDGET_UNKNOWNVAL));
	if (ch->PH > ch->maxPH)
