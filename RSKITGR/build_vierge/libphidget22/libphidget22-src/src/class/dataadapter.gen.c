...
_setStatus(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	if(hasBridgePacketByName(bp, "responseID"))
	...
	if (hasBridgePacketByName(bp, "baudRate"))
		ch->baudRate = getBridgePacketUInt32ByName(bp, "baudRate");
	if (hasBridgePacketByName(bp, "minBaudRate"))
		ch->minBaudRate = getBridgePacketUInt32ByName(bp, "minBaudRate");
	if (hasBridgePacketByName(bp, "maxBaudRate"))
	...
	if (hasBridgePacketByName(bp, "deviceAddress"))
		ch->deviceAddress = getBridgePacketUInt32ByName(bp, "deviceAddress");
	if (hasBridgePacketByName(bp, "handshakeMode"))
	...
	if (hasBridgePacketByName(bp, "maxSendWaitPacketLength"))
...
PhidgetDataAdapter_setI2CFormat(PhidgetDataAdapterHandle ch, const char *format) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETI2CFORMAT, NULL, NULL, 1, "%s", format); 
}
...
PhidgetDataAdapter_setEndOfLine(PhidgetDataAdapterHandle ch, const char *endOfLine) {
	...
	return bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETENDOFLINE, NULL, NULL, 1, "%s",
	  endOfLine); 
}
...
PhidgetDataAdapter_setBaudRate(PhidgetDataAdapterHandle ch, uint32_t baudRate) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETBAUDRATE, NULL, NULL, 1, "%u",
	  baudRate));
}
...
PhidgetDataAdapter_setDeviceAddress(PhidgetDataAdapterHandle ch, uint32_t deviceAddress) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETADDRESS, NULL, NULL, 1, "%u",
	  deviceAddress));
}
...
PhidgetDataAdapter_setHandshakeMode(PhidgetDataAdapterHandle ch,
  PhidgetDataAdapter_HandshakeMode handshakeMode) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETHANDSHAKEMODE, NULL, NULL, 1, "%d",
	  handshakeMode));
}
...
PhidgetDataAdapter_setParity(PhidgetDataAdapterHandle ch, PhidgetDataAdapter_Parity parity) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETPARITY, NULL, NULL, 1, "%d", parity));
}
...
PhidgetDataAdapter_setResponseTimeout(PhidgetDataAdapterHandle ch, uint32_t responseTimeout) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETTIMEOUT, NULL, NULL, 1, "%u",
	  responseTimeout));
}
...
PhidgetDataAdapter_setStopBits(PhidgetDataAdapterHandle ch, PhidgetDataAdapter_StopBits stopBits) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETSTOPBITS, NULL, NULL, 1, "%d",
	  stopBits));
}
...
PhidgetDataAdapter_setTransmitTimeout(PhidgetDataAdapterHandle ch, uint32_t transmitTimeout) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETTXTIMEOUT, NULL, NULL, 1, "%u",
	  transmitTimeout));
}
