...
PhidgetPressureSensor_getPressure(PhidgetPressureSensorHandle ch, double *pressure) {
	...
	*pressure = ch->pressure;
	if (ch->pressure == (double)PUNK_DBL || isnan(ch->pressure))
		return (PHID_RETURN(EPHIDGET_UNKNOWNVAL));
	if (ch->pressure > ch->maxPressure)
		return (PHID_RETURN(EPHIDGET_UNKNOWNVALHIGH));
	if (ch->pressure < ch->minPressure)
