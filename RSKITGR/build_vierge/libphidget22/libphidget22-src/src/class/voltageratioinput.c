	...
	if (supportedBridgePacket(phid, BP_SENSORCHANGE) && !isNetworkPhidget(ch) && ch->sensorType != SENSOR_TYPE_VOLTAGERATIO) {
		...
		if (!PhidgetAnalogSensor_getVoltageRatioSensorValueInRange(sensorValue, ch->sensorType)) {
			...
			if (ISATTACHEDDONE(ch))
				PhidgetChannel_sendErrorEventThrottled(phid, EEPHIDGET_OUTOFRANGE, "Sensor value is ouside the valid range for this sensor.");
		} else if (ch->sensorValue == PUNK_DBL || fabs(sensorValue - ch->sensorValue) >= ch->sensorValueChangeTrigger) {
			ch->sensorValue = sensorValue;
			if (ISATTACHEDDONE(ch)) {
				// NOTE: 4 entries so there is space for the unit info
				res = createBridgePacket(&bp, BP_SENSORCHANGE, 4, "%g", sensorValue);
				if (res == EPHIDGET_OK) {
		...
		if (res == EPHIDGET_OK) {
			...
			case PHIDUID_1046_1:
				...
				if (bridgePacketIsFromNet(bp)) {
					FIRE_PROPERTYCHANGE(ch, "MinVoltageRatio");
					FIRE_PROPERTYCHANGE(ch, "MaxVoltageRatio");
				} else {
					// We are running in the context of a set(), so we have to dispatch the PROPERTYCHANGE events
					DISPATCH_PROPERTYCHANGE(ch, "MinVoltageRatio");
					DISPATCH_PROPERTYCHANGE(ch, "MaxVoltageRatio");
				}
				break;
	#if PHIDUID_DAQ1501_SUPPORTED
			case PHIDUID_DAQ1501:
				switch (ch->bridgeGain) {
				case BRIDGE_GAIN_1:
					ch->maxVoltageRatio = 0.48;
					ch->minVoltageRatio = -0.48;
					break;
				case BRIDGE_GAIN_2:
					ch->maxVoltageRatio = 0.24;
					ch->minVoltageRatio = -0.24;
					break;
				case BRIDGE_GAIN_64:
					ch->maxVoltageRatio = 0.0075;
					ch->minVoltageRatio = -0.0075;
					break;
				case BRIDGE_GAIN_128:
					ch->maxVoltageRatio = 0.00375;
					ch->minVoltageRatio = -0.00375;
					break;
				}
