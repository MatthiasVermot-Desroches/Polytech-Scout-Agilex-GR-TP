...
_setStatus(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	if (hasBridgePacketByName(bp, "overVoltage"))
		ch->overVoltage = getBridgePacketDoubleByName(bp, "overVoltage");
	if (hasBridgePacketByName(bp, "minOverVoltage"))
		ch->minOverVoltage = getBridgePacketDoubleByName(bp, "minOverVoltage");
	if (hasBridgePacketByName(bp, "maxOverVoltage"))
...
_setDefaults(PhidgetChannelHandle phid) {
	...
	switch (phid->UCD->uid) {
	case PHIDCHUID_SAF1000_POWERGUARD_100:
		ret = bridgeSendToDevice(phid, BP_SETENABLED, NULL, NULL, 1, "%d", ch->powerEnabled);
		if (ret != EPHIDGET_OK)
			break;
		ret = bridgeSendToDevice(phid, BP_SETFANMODE, NULL, NULL, 1, "%d", ch->fanMode);
		if (ret != EPHIDGET_OK)
			break;
		break;
	case PHIDCHUID_SAF1000_POWERGUARD_110:
...
PhidgetPowerGuard_setOverVoltage(PhidgetPowerGuardHandle ch, double overVoltage) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETOVERVOLTAGE, NULL, NULL, 1, "%g",
	  overVoltage));
}
