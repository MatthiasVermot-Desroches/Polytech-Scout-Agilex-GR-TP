...
_initAfterOpen(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_100:
		...
		if (parentVINT->RTDWireSetup != PUNK_ENUM)
			ch->RTDWireSetup = parentVINT->RTDWireSetup;
		break;
	case PHIDCHUID_TMP1200_RESISTANCEINPUT_300:
		...
		ch->maxResistanceChangeTrigger = 5000;
...
PhidgetResistanceInput_getResistance(PhidgetResistanceInputHandle ch, double *resistance) {
	...
	*resistance = ch->resistance;
	if (ch->resistance == (double)PUNK_DBL || isnan(ch->resistance))
		return (PHID_RETURN(EPHIDGET_UNKNOWNVAL));
	if (ch->resistance > ch->maxResistance)
		return (PHID_RETURN(EPHIDGET_UNKNOWNVALHIGH));
	if (ch->resistance < ch->minResistance)
