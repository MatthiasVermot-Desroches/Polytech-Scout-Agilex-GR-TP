...
_getStatus(PhidgetChannelHandle phid, BridgePacket **bp) {
	...
	return (createBridgePacket(bp, BP_SETSTATUS, 13, "_class_version_=%u"
	  ",dataInterval=%u"
	  ",minDataInterval=%u"
	  ",maxDataInterval=%u"
	  ",dataIntervalDbl=%g"
...
PhidgetLightSensor_getIlluminance(PhidgetLightSensorHandle ch, double *illuminance) {
	...
	*illuminance = ch->illuminance;
	if (ch->illuminance == (double)PUNK_DBL || isnan(ch->illuminance))
		return (PHID_RETURN(EPHIDGET_UNKNOWNVAL));
	if (ch->illuminance > ch->maxIlluminance)
		return (PHID_RETURN(EPHIDGET_UNKNOWNVALHIGH));
	if (ch->illuminance < ch->minIlluminance)
