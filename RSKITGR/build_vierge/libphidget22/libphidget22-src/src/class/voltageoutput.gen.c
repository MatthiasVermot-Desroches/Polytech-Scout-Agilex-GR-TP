...
_setStatus(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	if (hasBridgePacketByName(bp, "voltageOutputRange"))
...
_getStatus(PhidgetChannelHandle phid, BridgePacket **bp) {
	...
	return (createBridgePacket(bp, BP_SETSTATUS, 8, "_class_version_=%u"
	  ",enabled=%d"
	  ",minFailsafeTime=%u"
	  ",maxFailsafeTime=%u"
	  ",voltage=%g"
...
_setDefaults(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_1002_VOLTAGEOUTPUT_100:
		...
		if (ret != EPHIDGET_OK)
			break;
		ret = bridgeSendToDevice(phid, BP_SETVOLTAGE, NULL, NULL, 1, "%g", ch->voltage);
		if (ret != EPHIDGET_OK)
			break;
		break;
	case PHIDCHUID_OUT1000_VOLTAGEOUTPUT_100:
	...
	case PHIDCHUID_OUT1000_VOLTAGEOUTPUT_110:
		...
		break;
	case PHIDCHUID_OUT1001_VOLTAGEOUTPUT_100:
		ret = bridgeSendToDevice(phid, BP_SETVOLTAGERANGE, NULL, NULL, 1, "%d",
		  ch->voltageOutputRange);
		if (ret != EPHIDGET_OK)
			break;
		break;
...
PhidgetVoltageOutput_setVoltage(PhidgetVoltageOutputHandle ch, double voltage) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETVOLTAGE, NULL, NULL, 1, "%g", voltage));
}
...
PhidgetVoltageOutput_setVoltage_async(PhidgetVoltageOutputHandle ch, double voltage,
  Phidget_AsyncCallback fptr, void *ctx) {
	...
	res = bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETVOLTAGE, fptr, ctx, 1, "%g", voltage);
