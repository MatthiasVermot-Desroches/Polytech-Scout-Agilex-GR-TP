...
PhidgetHumiditySensor_getHumidity(PhidgetHumiditySensorHandle ch, double *humidity) {
	...
	*humidity = ch->humidity;
	if (ch->humidity == (double)PUNK_DBL || isnan(ch->humidity))
		return (PHID_RETURN(EPHIDGET_UNKNOWNVAL));
	if (ch->humidity > ch->maxHumidity)
		return (PHID_RETURN(EPHIDGET_UNKNOWNVALHIGH));
	if (ch->humidity < ch->minHumidity)
