...
_setStatus(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	if(hasBridgePacketByName(bp, "amplitudes"))
		memcpy(&ch->amplitudes, getBridgePacketUInt32ArrayByName(bp, "amplitudes"),
	  sizeof (uint32_t) * 8);
	if(hasBridgePacketByName(bp, "distances"))
...
_setDefaults(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_DST1200_DISTANCESENSOR_100:
		...
		if (ret != EPHIDGET_OK)
			break;
		ret = bridgeSendToDevice(phid, BP_SETSONARQUIETMODE, NULL, NULL, 1, "%d", ch->sonarQuietMode);
		if (ret != EPHIDGET_OK)
			break;
		break;
	default:
...
PhidgetDistanceSensor_getDistance(PhidgetDistanceSensorHandle ch, uint32_t *distance) {
	...
	*distance = ch->distance;
	if (ch->distance == (uint32_t)PUNK_UINT32)
		return (PHID_RETURN(EPHIDGET_UNKNOWNVAL));
	if (ch->distance > ch->maxDistance)
		return (PHID_RETURN(EPHIDGET_UNKNOWNVALHIGH));
	if (ch->distance < ch->minDistance)
...
PhidgetDistanceSensor_setSonarQuietMode(PhidgetDistanceSensorHandle ch, int sonarQuietMode) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETSONARQUIETMODE, NULL, NULL, 1, "%d",
	  sonarQuietMode));
}
