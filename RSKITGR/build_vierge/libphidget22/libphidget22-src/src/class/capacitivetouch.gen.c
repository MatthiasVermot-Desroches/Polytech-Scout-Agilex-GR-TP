...
_setDefaults(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_HIN1000_CAPACITIVETOUCH_100:
		...
		if (ret != EPHIDGET_OK)
			break;
		ret = bridgeSendToDevice(phid, BP_SETSENSITIVITY, NULL, NULL, 1, "%g", ch->sensitivity);
		if (ret != EPHIDGET_OK)
			break;
		break;
	case PHIDCHUID_HIN1001_CAPACITIVETOUCH_BUTTONS_100:
...
PhidgetCapacitiveTouch_setSensitivity(PhidgetCapacitiveTouchHandle ch, double sensitivity) {
	...
	return (bridgeSendToDevice((PhidgetChannelHandle)ch, BP_SETSENSITIVITY, NULL, NULL, 1, "%g",
	  sensitivity));
}
